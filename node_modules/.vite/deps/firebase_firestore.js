...
var M = "11.7.3";
var x = new Logger("@firebase/firestore");
...
function __PRIVATE_logDebug(e, ...t) {
  if (x.logLevel <= LogLevel.DEBUG) {
    ...
    x.debug(`Firestore (${M}): ${e}`, ...n);
  }
}
function __PRIVATE_logError(e, ...t) {
  if (x.logLevel <= LogLevel.ERROR) {
    const n = t.map(__PRIVATE_argToString);
    x.error(`Firestore (${M}): ${e}`, ...n);
  }
}
function __PRIVATE_logWarn(e, ...t) {
  if (x.logLevel <= LogLevel.WARN) {
    const n = t.map(__PRIVATE_argToString);
    x.warn(`Firestore (${M}): ${e}`, ...n);
  }
}
...
function __PRIVATE__fail(e, t, n) {
  let r = `FIRESTORE (${M}) INTERNAL ASSERTION FAILED: ${t} (ID: ${e.toString(16)})`;
  if (void 0 !== n)
    try {
...
var Timestamp = class _Timestamp {
   ...
   */
  static fromMillis(e) {
    const t = Math.floor(e / 1e3), n = Math.floor((e - 1e3 * t) * B);
  ...
  constructor(e, t) {
    if (this.seconds = e, this.nanoseconds = t, t < 0)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
    if (t >= 1e9)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
    if (e < N)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
    if (e >= 253402300800)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
  }
  /**
...
var ResourcePath = class _ResourcePath extends BasePath {
  ...
  static fromString(...e) {
    ...
    for (const n of e) {
      if (n.indexOf("//") >= 0)
        throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid segment (${n}). Paths must not contain // in them.`);
...
var FieldPath$1 = class _FieldPath$1 extends BasePath {
  ...
  static fromServerFormat(e) {
    ...
    const __PRIVATE_addCurrentSegment = () => {
      if (0 === n.length)
        throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid field path (${e}). Paths must not be empty, begin with '.', end with '.', or contain '..'`);
      t.push(n), n = "";
    };
    let i = false;
    for (; r < e.length; ) {
      ...
      if ("\\" === t2) {
        if (r + 1 === e.length)
          throw new FirestoreError(O.INVALID_ARGUMENT, "Path has trailing escape character: " + e);
        const t3 = e[r + 1];
        if ("\\" !== t3 && "." !== t3 && "`" !== t3)
          throw new FirestoreError(O.INVALID_ARGUMENT, "Path has invalid escape sequence: " + e);
        n += t3, r += 2;
      } else
        "`" === t2 ? (i = !i, r++) : "." !== t2 || i ? (n += t2, r++) : (__PRIVATE_addCurrentSegment(), r++);
    }
    if (__PRIVATE_addCurrentSegment(), i)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Unterminated ` in path: " + e);
...
var Q = "The current tab is not in the required state to perform this operation. It might be necessary to refresh the browser tab.";
...
async function __PRIVATE_ignoreIfPrimaryLeaseLoss(e) {
  if (e.code !== O.FAILED_PRECONDITION || e.message !== Q)
    throw e;
...
var __PRIVATE_SimpleDbTransaction = class ___PRIVATE_SimpleDbTransaction {
  ...
  abort(e) {
    e && this.S.reject(e), this.aborted || (__PRIVATE_logDebug($, "Aborting transaction:", e ? e.message : "Client-initiated abort"), this.aborted = true, this.transaction.abort());
  }
  v() {
...
var __PRIVATE_SimpleDb = class ___PRIVATE_SimpleDb {
  ...
  static delete(e) {
    __PRIVATE_logDebug($, "Removing database:", e);
   ...
   */
  async k(e) {
    return this.db || (__PRIVATE_logDebug($, "Opening database:", this.name), this.db = await new Promise((t, n) => {
      ...
      r.onsuccess = (e2) => {
        ...
        "VersionError" === r2.name ? n(new FirestoreError(O.FAILED_PRECONDITION, "A newer version of the Firestore SDK was previously used and so the persisted data is not compatible with the version of the SDK you are now using. The SDK will operate with persistence disabled. If you need persistence, please re-upgrade to a newer version of the SDK or else clear the persisted IndexedDB data for your app to start fresh.")) : "InvalidStateError" === r2.name ? n(new FirestoreError(O.FAILED_PRECONDITION, "Unable to open an IndexedDB connection. This could be due to running in a private browsing session on a browser whose private browsing sessions do not support IndexedDB: " + r2)) : n(new __PRIVATE_IndexedDbTransactionError(e, r2));
      }, r.onupgradeneeded = (e2) => {
        __PRIVATE_logDebug($, 'Database "' + this.name + '" requires upgrade from version:', e2.oldVersion);
        ...
        this.B.q(t2, r.transaction, e2.oldVersion, this.version).next(() => {
          __PRIVATE_logDebug($, "Database upgrade to version " + this.version + " complete");
        });
      };
  ...
  async runTransaction(e, t, n, r) {
    ...
    for (; ; ) {
      ...
      try {
        ...
        if (__PRIVATE_logDebug($, "Transaction failed with error:", t2.message, "Retrying:", n2), this.close(), !n2)
...
var __PRIVATE_IndexedDbTransactionError = class extends FirestoreError {
  constructor(e, t) {
    super(O.UNAVAILABLE, `IndexedDB transaction '${e}' failed: ${t}`), this.name = "IndexedDbTransactionError";
  }
};
...
var __PRIVATE_SimpleDbStore = class {
  ...
  put(e, t) {
    let n;
    return void 0 !== t ? (__PRIVATE_logDebug($, "PUT", this.store.name, e, t), n = this.store.put(t, e)) : (__PRIVATE_logDebug($, "PUT", this.store.name, "<auto-key>", e), n = this.store.put(e)), __PRIVATE_wrapRequest(n);
  }
  /**
   ...
   */
  add(e) {
    __PRIVATE_logDebug($, "ADD", this.store.name, e, e);
  ...
  get(e) {
    return __PRIVATE_wrapRequest(this.store.get(e)).next((t) => (
      ...
      (void 0 === t && (t = null), __PRIVATE_logDebug($, "GET", this.store.name, e, t), t)
    ));
  }
  delete(e) {
    __PRIVATE_logDebug($, "DELETE", this.store.name, e);
   ...
   */
  count() {
    __PRIVATE_logDebug($, "COUNT", this.store.name);
  ...
  X(e, t) {
    __PRIVATE_logDebug($, "DELETE ALL", this.store.name);
...
function __PRIVATE_checkForAndReportiOSError(e) {
  ...
  if (t >= 12.2 && t < 13) {
    ...
    if (e.message.indexOf(t2) >= 0) {
      ...
      return U || (U = true, // Throw a global exception outside of this promise chain, for the user to
      // potentially catch.
      setTimeout(() => {
        throw e2;
      }, 0)), e2;
...
var __PRIVATE_IndexBackfiller = class {
  ...
  oe(e, t) {
    ...
    return PersistencePromise.doWhile(() => true === i && r > 0, () => this.localStore.indexManager.getNextCollectionGroupToUpdate(e).next((t2) => {
      if (null !== t2 && !n.has(t2))
        return __PRIVATE_logDebug(K, `Processing collection: ${t2}`), this._e(e, t2, r).next((e2) => {
          r -= e2, n.add(t2);
        });
      i = false;
    })).next(() => t - r);
  ...
  _e(e, t, n) {
    return this.localStore.indexManager.getMinOffsetFromCollectionGroup(e, t).next((r) => this.localStore.localDocuments.getNextDocuments(e, t, r, n).next((n2) => {
      const i = n2.changes;
      return this.localStore.indexManager.updateIndexEntries(e, i).next(() => this.ae(r, n2)).next((n3) => (__PRIVATE_logDebug(K, `Updating offset: ${n3}`), this.localStore.indexManager.updateCollectionGroup(e, t, n3))).next(() => i.size);
    }));
  }
...
function __PRIVATE_decodeResourcePath(e) {
  ...
  if (__PRIVATE_hardAssert(t >= 2, 64408, {
    ...
    return __PRIVATE_hardAssert(e.charAt(0) === G && "" === e.charAt(1), 56145, {
      path: e
    }), ResourcePath.emptyPath();
  const __PRIVATE_lastReasonableEscapeIndex = t - 2, n = [];
  let r = "";
  for (let i = 0; i < t; ) {
    const t2 = e.indexOf(G, i);
...
var z = "remoteDocuments";
var j = "owner";
var H = "owner";
var J = "mutationQueues";
var Y = "userId";
var Z = "mutations";
var X = "batchId";
var ee = "userMutationsIndex";
var te = ["userId", "batchId"];
...
var ne = {};
var re = "documentMutations";
var ie = "remoteDocumentsV14";
var se = ["prefixPath", "collectionGroup", "readTime", "documentId"];
var oe = "documentKeyIndex";
var _e = ["prefixPath", "collectionGroup", "documentId"];
var ae = "collectionGroupIndex";
var ue = ["collectionGroup", "readTime", "prefixPath", "documentId"];
var ce = "remoteDocumentGlobal";
var le = "remoteDocumentGlobalKey";
var he = "targets";
var Pe = "queryTargetsIndex";
var Te = ["canonicalId", "targetId"];
var Ie = "targetDocuments";
var Ee = ["targetId", "path"];
var de = "documentTargetsIndex";
var Ae = ["path", "targetId"];
var Re = "targetGlobalKey";
var Ve = "targetGlobal";
var me = "collectionParents";
var fe = ["collectionId", "parent"];
var ge = "clientMetadata";
var pe = "clientId";
var ye = "bundles";
var we = "bundleId";
var be = "namedQueries";
var Se = "name";
var De = "indexConfiguration";
var ve = "indexId";
var Ce = "collectionGroupIndex";
var Fe = "collectionGroup";
var Me = "indexState";
var xe = ["indexId", "uid"];
var Oe = "sequenceNumberIndex";
var Ne = ["uid", "sequenceNumber"];
var Be = "indexEntries";
var Le = ["indexId", "uid", "arrayValue", "directionalValue", "orderedDocumentKey", "documentKey"];
var ke = "documentKeyIndex";
var qe = ["indexId", "uid", "orderedDocumentKey"];
var Qe = "documentOverlays";
var $e = ["userId", "collectionPath", "documentId"];
var Ue = "collectionPathOverlayIndex";
var Ke = ["userId", "collectionPath", "largestBatchId"];
var We = "collectionGroupOverlayIndex";
var Ge = ["userId", "collectionGroup", "largestBatchId"];
var ze = "globals";
var je = "name";
var He = [...[...[...[...[J, Z, re, z, he, j, Ve, Ie], ge], ce], me], ye, be];
var Je = [...He, Qe];
var Ye = [J, Z, re, ie, he, j, Ve, Ie, ge, ce, me, ye, be, Qe];
var Ze = Ye;
var Xe = [...Ze, De, Me, Be];
var et = Xe;
var tt = [...Xe, ze];
...
var rt = "server_timestamp";
var it = "__type__";
var st = "__previous_value__";
var ot = "__local_write_time__";
function __PRIVATE_isServerTimestamp(e) {
  var t, n;
  return (null === (n = ((null === (t = null == e ? void 0 : e.mapValue) || void 0 === t ? void 0 : t.fields) || {})[it]) || void 0 === n ? void 0 : n.stringValue) === rt;
}
function __PRIVATE_getPreviousValue(e) {
  const t = e.mapValue.fields[st];
  return __PRIVATE_isServerTimestamp(t) ? __PRIVATE_getPreviousValue(t) : t;
}
function __PRIVATE_getLocalWriteTime(e) {
  const t = __PRIVATE_normalizeTimestamp(e.mapValue.fields[ot].timestampValue);
...
var DatabaseId = class _DatabaseId {
  constructor(e, t) {
    this.projectId = e, this.database = t || _t;
  }
  static empty() {
    return new _DatabaseId("", "");
  }
  get isDefaultDatabase() {
    return this.database === _t;
  }
  isEqual(e) {
  ...
  switch (n) {
    ...
    case 10:
      return function __PRIVATE_compareVectors(e2, t2) {
        var n2, r2, i, s;
        const o = e2.fields || {}, _ = t2.fields || {}, a = null === (n2 = o[ht]) || void 0 === n2 ? void 0 : n2.arrayValue, u = null === (r2 = _[ht]) || void 0 === r2 ? void 0 : r2.arrayValue, c = __PRIVATE_primitiveComparator((null === (i = null == a ? void 0 : a.values) || void 0 === i ? void 0 : i.length) || 0, (null === (s = null == u ? void 0 : u.values) || void 0 === s ? void 0 : s.length) || 0);
        if (0 !== c)
          return c;
    ...
    case 11:
      return function __PRIVATE_compareMaps(e2, t2) {
        if (e2 === ct.mapValue && t2 === ct.mapValue)
          return 0;
        if (e2 === ct.mapValue)
          return 1;
        if (t2 === ct.mapValue)
...
function __PRIVATE_isVectorValue(e) {
  var t, n;
  return (null === (n = ((null === (t = null == e ? void 0 : e.mapValue) || void 0 === t ? void 0 : t.fields) || {})[at]) || void 0 === n ? void 0 : n.stringValue) === lt;
}
...
function __PRIVATE_isMaxValue(e) {
  return (((e.mapValue || {}).fields || {}).__type__ || {}).stringValue === ut;
}
var Tt = {
  mapValue: {
    fields: {
...
function __PRIVATE_valuesGetLowerBound(e) {
  return "nullValue" in e ? Pt : "booleanValue" in e ? {
    booleanValue: false
...
var At = new SortedSet(DocumentKey.comparator);
...
function __PRIVATE_applyTransformOperationToLocalView(e, t, n) {
  return e instanceof __PRIVATE_ServerTimestampTransform ? function serverTimestamp$1(e2, t2) {
    ...
    return t2 && __PRIVATE_isServerTimestamp(t2) && (t2 = __PRIVATE_getPreviousValue(t2)), t2 && (n2.fields[st] = t2), {
      mapValue: n2
    };
...
function __PRIVATE_mapCodeFromRpcCode(e) {
  if (void 0 === e)
    return __PRIVATE_logError("GRPC error has no .code"), O.UNKNOWN;
  switch (e) {
    case Vt.OK:
      return O.OK;
    case Vt.CANCELLED:
...
(mt = Vt || (Vt = {}))[mt.OK = 0] = "OK", mt[mt.CANCELLED = 1] = "CANCELLED", mt[mt.UNKNOWN = 2] = "UNKNOWN", mt[mt.INVALID_ARGUMENT = 3] = "INVALID_ARGUMENT", mt[mt.DEADLINE_EXCEEDED = 4] = "DEADLINE_EXCEEDED", mt[mt.NOT_FOUND = 5] = "NOT_FOUND", mt[mt.ALREADY_EXISTS = 6] = "ALREADY_EXISTS", mt[mt.PERMISSION_DENIED = 7] = "PERMISSION_DENIED", mt[mt.UNAUTHENTICATED = 16] = "UNAUTHENTICATED", mt[mt.RESOURCE_EXHAUSTED = 8] = "RESOURCE_EXHAUSTED", mt[mt.FAILED_PRECONDITION = 9] = "FAILED_PRECONDITION", mt[mt.ABORTED = 10] = "ABORTED", mt[mt.OUT_OF_RANGE = 11] = "OUT_OF_RANGE", mt[mt.UNIMPLEMENTED = 12] = "UNIMPLEMENTED", mt[mt.INTERNAL = 13] = "INTERNAL", mt[mt.UNAVAILABLE = 14] = "UNAVAILABLE", mt[mt.DATA_LOSS = 15] = "DATA_LOSS";
var ft = null;
var gt = new Integer([4294967295, 4294967295], 0);
...
var BloomFilter = class _BloomFilter {
  ...
  we(e, t, n) {
    ...
    return 1 === r.compare(gt) && (r = new Integer([r.getBits(0), r.getBits(1)], 0)), r.modulo(this.ye).toNumber();
  }
    ...
    if (r) {
      ...
      if (__PRIVATE_targetIsDocumentTarget(i))
        ...
        if (r2 !== n) {
          ...
          null == ft || ft.ht(function __PRIVATE_createExistenceFilterMismatchInfoForTestingHooks(e2, t2, n3, r3, i3) {
            var s, o, _, a, u, c;
            const l = {
              localCacheCount: e2,
...
function fromName(e, t) {
  ...
  if (n.get(1) !== e.databaseId.projectId)
    throw new FirestoreError(O.INVALID_ARGUMENT, "Tried to deserialize key from different project: " + n.get(1) + " vs " + e.databaseId.projectId);
  if (n.get(3) !== e.databaseId.database)
    throw new FirestoreError(O.INVALID_ARGUMENT, "Tried to deserialize key from different database: " + n.get(3) + " vs " + e.databaseId.database);
...
function __PRIVATE_fromWatchChange(e, t) {
  ...
  if ("targetChange" in t) {
    ...
    const r = function __PRIVATE_fromWatchTargetChangeState(e2) {
      ...
      const t2 = void 0 === e2.code ? O.UNKNOWN : __PRIVATE_mapCodeFromRpcCode(e2.code);
...
var __PRIVATE_IndexedDbDocumentOverlayCache = class ___PRIVATE_IndexedDbDocumentOverlayCache {
  ...
  removeOverlaysForBatchId(e, t, n) {
    ...
    return r.forEach((t2) => {
      ...
      i.push(__PRIVATE_documentOverlayStore(e).X(Ue, r2));
  ...
  getOverlaysForCollection(e, t, n) {
    ...
    return __PRIVATE_documentOverlayStore(e).J(Ue, s).next((e2) => {
      for (const t2 of e2) {
...
var __PRIVATE_IndexedDbGlobalsCache = class {
  Dt(e) {
    return __PRIVATE_getStore(e, ze);
  }
  getSessionToken(e) {
...
var __PRIVATE_FirestoreIndexValueWriter = class {
  ...
  qt(e, t) {
    ...
    const i = e.fields || {};
    this.Mt(t, 53);
    const s = ht, o = (null === (r = null === (n = i[s].arrayValue) || void 0 === n ? void 0 : n.values) || void 0 === r ? void 0 : r.length) || 0;
...
var __PRIVATE_OrderedCodeWriter = class {
  ...
  zt(e) {
    const t = 255 & e;
    0 === t ? (this._n(0), this._n(255)) : t === bt ? (this._n(bt), this._n(0)) : this._n(t);
  }
  /** Writes a single byte descending to the buffer.  */
  Jt(e) {
    const t = 255 & e;
    0 === t ? (this.un(0), this.un(255)) : t === bt ? (this.un(bt), this.un(0)) : this.un(e);
  }
  jt() {
    this._n(0), this._n(1);
  }
...
var __PRIVATE_TargetIndexMatcher = class {
  constructor(e) {
    this.En = new SortedSet((e2, t) => FieldPath$1.comparator(e2.field, t.field)), this.collectionId = null != e.collectionGroup ? e.collectionGroup : e.path.lastSegment(), this.dn = e.orderBy, this.An = [];
    for (const t of e.filters) {
      const e2 = t;
      e2.isInequality() ? this.En = this.En.add(e2) : this.An.push(e2);
    }
  }
  get Rn() {
   ...
   */
  Vn(e) {
    if (__PRIVATE_hardAssert(e.collectionGroup === this.collectionId, 49279), this.Rn)
      return false;
    ...
    for (; i < n.length && this.mn(n[i]); ++i)
      r = r.add(n[i].fieldPath.canonicalString());
    if (i === n.length)
      return true;
    if (this.En.size > 0) {
      const e2 = this.En.getIterator().getNext();
      if (!r.has(e2.field.canonicalString())) {
        const t2 = n[i];
        if (!this.fn(e2, t2) || !this.gn(this.dn[s++], t2))
          return false;
      }
      ++i;
    }
    for (; i < n.length; ++i) {
      const e2 = n[i];
      if (s >= this.dn.length || !this.gn(this.dn[s++], e2))
        return false;
    }
    return true;
  }
...
var __PRIVATE_IndexedDbIndexManager = class {
  constructor(e, t) {
     ...
     */
    this.wn = new __PRIVATE_MemoryCollectionParentIndex(), /**
     * Maps from a target to its equivalent list of sub-targets. Each sub-target
     * contains only one term from the target's disjunctive normal form (DNF).
     */
    this.bn = new ObjectMap((e2) => __PRIVATE_canonifyTarget(e2), (e2, t2) => __PRIVATE_targetEquals(e2, t2)), this.uid = e.uid || "";
  }
  /**
  ...
  createTargetIndexes(e, t) {
    return PersistencePromise.forEach(this.Sn(t), (t2) => this.getIndexType(e, t2).next((n) => {
      if (0 === n || 1 === n) {
  ...
  getDocumentsMatchingTarget(e, t) {
    ...
    return PersistencePromise.forEach(this.Sn(t), (t2) => this.Dn(e, t2).next((e2) => {
      r && (r = !!e2), i.set(t2, e2);
    })).next(() => {
      if (r) {
        ...
        return PersistencePromise.forEach(i, (i2, s) => {
          __PRIVATE_logDebug(St, `Using index ${function __PRIVATE_fieldIndexToString(e3) {
          ...
          const o = function __PRIVATE_targetGetArrayValues(e3, t2) {
            ...
            return new Bound(n2, r3);
          }(s, i2), c = this.vn(i2, s, a), l = this.vn(i2, s, u), h = this.Cn(i2, s, _), P = this.Fn(i2.indexId, o, c, a.inclusive, l, u.inclusive, h);
  ...
  Fn(e, t, n, r, i, s, o) {
    ...
    for (let c = 0; c < _; ++c) {
      const _2 = t ? this.Mn(t[c / a]) : Dt, l = this.xn(e, _2, n[c % a], r), h = this.On(e, _2, i[c % a], s), P = o.map((t2) => this.xn(
        e,
        _2,
        t2,
        /* inclusive= */
  ...
  Dn(e, t) {
    ...
    return this.getFieldIndexes(e, r).next((e2) => {
      let t2 = null;
      for (const r2 of e2) {
        n.Vn(r2) && (!t2 || r2.fields.length > t2.fields.length) && (t2 = r2);
      }
      return t2;
    });
  }
  getIndexType(e, t) {
    let n = 2;
    const r = this.Sn(t);
    return PersistencePromise.forEach(r, (t2) => this.Dn(e, t2).next((e2) => {
  ...
  Cn(e, t, n) {
    ...
    for (const s of __PRIVATE_fieldIndexGetDirectionalSegments(e)) {
      const e2 = n[i++];
      for (const n2 of r)
        if (this.Ln(t, s.fieldPath) && isArray(e2))
          r = this.kn(r, s, e2);
        else {
  ...
  getFieldIndexes(e, t) {
    ...
    return (t ? n.J(Ce, IDBKeyRange.bound(t, t)) : n.J()).next((e2) => {
      const t2 = [];
  ...
  updateCollectionGroup(e, t, n) {
    ...
    return this.Qn(e).next((e2) => r.J(Ce, IDBKeyRange.bound(t, t)).next((t2) => PersistencePromise.forEach(t2, (t3) => i.put(__PRIVATE_toDbIndexState(t3.indexId, this.uid, e2, n)))));
  }
  updateIndexEntries(e, t) {
    const n = /* @__PURE__ */ new Map();
    return PersistencePromise.forEach(t, (t2, r) => {
      const i = n.get(t2.collectionGroup);
      return (i ? PersistencePromise.resolve(i) : this.getFieldIndexes(e, t2.collectionGroup)).next((i2) => (n.set(t2.collectionGroup, i2), PersistencePromise.forEach(i2, (n2) => this.$n(e, t2, n2).next((t3) => {
        const i3 = this.Un(r, n2);
        return t3.isEqual(i3) ? PersistencePromise.resolve() : this.Kn(e, r, n2, t3, i3);
      }))));
    });
  }
  Wn(e, t, n, r) {
    return __PRIVATE_indexEntriesStore(e).put({
      ...
      orderedDocumentKey: this.Bn(n, t.key),
      documentKey: t.key.path.toArray()
    });
  }
  Gn(e, t, n, r) {
    return __PRIVATE_indexEntriesStore(e).delete([r.indexId, this.uid, r.arrayValue, r.directionalValue, this.Bn(n, t.key), t.key.path.toArray()]);
  }
  $n(e, t, n) {
    ...
    return r.te({
      index: ke,
      range: IDBKeyRange.only([n.indexId, this.uid, this.Bn(n, t)])
    }, (e2, r2) => {
    ...
    if (null != i) {
      ...
      if (isArray(s))
        for (const i2 of s.arrayValue.values || [])
          n = n.add(new __PRIVATE_IndexEntry(t.indexId, e.key, this.Mn(i2), r));
    } else
      n = n.add(new __PRIVATE_IndexEntry(t.indexId, e.key, Dt, r));
    return n;
  }
  /**
   ...
   */
  Kn(e, t, n, r, i) {
    __PRIVATE_logDebug(St, "Updating index entries for document '%s'", t.key);
    const s = [];
  ...
  createRange(e, t, n) {
    ...
    for (let e2 = 0; e2 < r.length; e2 += 2) {
      if (this.zn(r[e2], r[e2 + 1]))
        return [];
      const t2 = [r[e2].indexId, this.uid, r[e2].arrayValue, r[e2].directionalValue, Dt, []], n2 = [r[e2 + 1].indexId, this.uid, r[e2 + 1].arrayValue, r[e2 + 1].directionalValue, Dt, []];
  ...
  getMinOffset(e, t) {
    return PersistencePromise.mapArray(this.Sn(t), (t2) => this.Dn(e, t2).next((e2) => e2 || fail(44426))).next(__PRIVATE_getMinOffsetFromFieldIndexes);
  }
};
...
function removeMutationBatch(e, t, n) {
  const r = e.store(Z), i = e.store(re), s = [], o = IDBKeyRange.only(n.batchId);
  let _ = 0;
  const a = r.te({
    range: o
...
LruParams.DEFAULT_COLLECTION_PERCENTILE = 10, LruParams.DEFAULT_MAX_SEQUENCE_NUMBERS_TO_COLLECT = 1e3, LruParams.DEFAULT = new LruParams(Ct, LruParams.DEFAULT_COLLECTION_PERCENTILE, LruParams.DEFAULT_MAX_SEQUENCE_NUMBERS_TO_COLLECT), LruParams.DISABLED = new LruParams(-1, 0, 0);
var __PRIVATE_IndexedDbMutationQueue = class ___PRIVATE_IndexedDbMutationQueue {
  ...
  addMutationBatch(e, t, n, r) {
    ...
    return s.add({}).next((o) => {
      ...
      for (const e2 of r) {
        ...
        c = c.add(e2.key.path.popLast()), u.push(s.put(a)), u.push(i.put(t2, ne));
      }
      return c.forEach((t2) => {
  ...
  // PORTING NOTE: Multi-tab only.
  Hn(e, t) {
    return this.jn[t] ? PersistencePromise.resolve(this.jn[t]) : this.lookupMutationBatch(e, t).next((e2) => {
      if (e2) {
        const n = e2.keys();
        return this.jn[t] = n, n;
      }
      return null;
    });
  }
  ...
  getAllMutationBatches(e) {
    const t = IDBKeyRange.bound([this.userId, W], [this.userId, Number.POSITIVE_INFINITY]);
    return __PRIVATE_mutationsStore(e).J(ee, t).next((e2) => e2.map((e3) => __PRIVATE_fromDbMutationBatch(this.serializer, e3)));
  }
  ...
  getAllMutationBatchesAffectingDocumentKeys(e, t) {
    ...
    return t.forEach((t2) => {
      ...
      r.push(o);
    }), PersistencePromise.waitFor(r).next(() => this.Zn(e, n));
  }
    ...
    return this.checkEmpty(e).next((t) => {
      ...
      return __PRIVATE_documentMutationsStore(e).te({
        ...
        __PRIVATE_hardAssert(0 === r.length, 56720, {
          er: r.map((e2) => e2.canonicalString())
        });
      });
    });
  }
...
var __PRIVATE_IndexedDbTargetCache = class {
  ...
  allocateTargetId(e) {
    return this.sr(e).next((t) => {
      ...
      return t.highestTargetId = n.next(), this._r(e, t).next(() => t.highestTargetId);
    });
  }
  getLastRemoteSnapshotVersion(e) {
    return this.sr(e).next((e2) => SnapshotVersion.fromTimestamp(new Timestamp(e2.lastRemoteSnapshotVersion.seconds, e2.lastRemoteSnapshotVersion.nanoseconds)));
  }
  getHighestSequenceNumber(e) {
    return this.sr(e).next((e2) => e2.highestListenSequenceNumber);
  }
  setTargetsMetadata(e, t, n) {
    return this.sr(e).next((r) => (r.highestListenSequenceNumber = t, n && (r.lastRemoteSnapshotVersion = n.toTimestamp()), t > r.highestListenSequenceNumber && (r.highestListenSequenceNumber = t), this._r(e, r)));
  }
  addTargetData(e, t) {
    return this.ar(e, t).next(() => this.sr(e).next((n) => (n.targetCount += 1, this.ur(t, n), this._r(e, n))));
  }
  updateTargetData(e, t) {
    return this.ar(e, t);
  }
  removeTargetData(e, t) {
    return this.removeMatchingKeysForTargetId(e, t.targetId).next(() => __PRIVATE_targetsStore(e).delete(t.targetId)).next(() => this.sr(e)).next((t2) => (__PRIVATE_hardAssert(t2.targetCount > 0, 8065), t2.targetCount -= 1, this._r(e, t2)));
  }
  /**
  ...
  getTargetCount(e) {
    return this.sr(e).next((e2) => e2.targetCount);
  }
  getTargetData(e, t) {
...
var __PRIVATE_RollingSequenceNumberBuffer = class {
  constructor(e) {
    this.cr = e, this.buffer = new SortedSet(__PRIVATE_bufferEntryComparator), this.lr = 0;
  }
  hr() {
    return ++this.lr;
  }
...
var __PRIVATE_LruScheduler = class {
  ...
  Ir(e) {
    __PRIVATE_logDebug(Ft, `Garbage collection scheduled in ${e}ms`), this.Tr = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection", e, async () => {
      this.Tr = null;
      try {
        await this.localStore.collectGarbage(this.garbageCollector);
      } catch (e2) {
        __PRIVATE_isIndexedDbTransactionError(e2) ? __PRIVATE_logDebug(Ft, "Ignoring IndexedDB error during garbage collection: ", e2) : await __PRIVATE_ignoreIfPrimaryLeaseLoss(e2);
      }
      await this.Ir(3e5);
    });
  }
...
var __PRIVATE_LruGarbageCollectorImpl = class {
  constructor(e, t) {
    this.Er = e, this.params = t;
  }
  calculateTargetCount(e, t) {
    return this.Er.dr(e).next((e2) => Math.floor(t / 100 * e2));
  }
  nthSequenceNumber(e, t) {
    if (0 === t)
      return PersistencePromise.resolve(__PRIVATE_ListenSequence.le);
    const n = new __PRIVATE_RollingSequenceNumberBuffer(t);
    return this.Er.forEachTarget(e, (e2) => n.Pr(e2.sequenceNumber)).next(() => this.Er.Ar(e, (e2) => n.Pr(e2))).next(() => n.maxValue);
  }
  removeTargets(e, t, n) {
    return this.Er.removeTargets(e, t, n);
  }
...
var __PRIVATE_IndexedDbLruDelegateImpl = class {
  ...
  removeOrphanedDocuments(e, t) {
    ...
    let i = 0;
    return this.mr(e, (s, o) => {
      if (o <= t) {
        const t2 = this.gr(e, s).next((t3) => {
          if (!t3)
...
var __PRIVATE_IndexedDbRemoteDocumentCacheImpl = class {
   ...
   */
  updateMetadata(e, t) {
    return this.getMetadata(e).next((n) => (n.byteSize += t, this.pr(e, n)));
  }
  getEntry(e, t) {
  ...
  getEntries(e, t) {
    ...
    return this.br(e, t, (e2, t2) => {
      const r = this.yr(e2, t2);
      n = n.insert(e2, r);
    }).next(() => n);
  }
  /**
  ...
  getDocumentsMatchingQuery(e, t, n, r, i) {
    ...
    return __PRIVATE_remoteDocumentsStore(e).J(IDBKeyRange.bound(o, _, true)).next((e2) => {
      ...
      for (const i2 of e2) {
        const e3 = this.yr(DocumentKey.fromSegments(i2.prefixPath.concat(i2.collectionGroup, i2.documentId)), i2);
  ...
  getAllFromCollectionGroup(e, t, n, r) {
    ...
    return __PRIVATE_remoteDocumentsStore(e).te({
      ...
      const s2 = this.yr(DocumentKey.fromSegments(t2.prefixPath.concat(t2.collectionGroup, t2.documentId)), t2);
  ...
  getMetadata(e) {
    return __PRIVATE_documentGlobalStore(e).get(le).next((e2) => (__PRIVATE_hardAssert(!!e2, 20021), e2));
  }
  pr(e, t) {
    return __PRIVATE_documentGlobalStore(e).put(le, t);
  }
  /**
...
var __PRIVATE_IndexedDbRemoteDocumentChangeBuffer = class extends RemoteDocumentChangeBuffer {
   ...
   */
  constructor(e, t) {
    super(), this.vr = e, this.trackRemovals = t, // A map of document sizes and read times prior to applying the changes in
    // this buffer.
  ...
  applyChanges(e) {
    ...
    return this.changes.forEach((i, s) => {
      ...
      if (t.push(this.vr.removeEntry(e, i, o.readTime)), s.isValidDocument()) {
        const _ = __PRIVATE_toDbRemoteDocument(this.vr.serializer, s);
        r = r.add(i.path.popLast());
        const a = __PRIVATE_dbDocumentSize(_);
        n += a - o.size, t.push(this.vr.addEntry(e, i, _));
      } else if (n -= o.size, this.trackRemovals) {
        const n2 = __PRIVATE_toDbRemoteDocument(this.vr.serializer, s.convertToNoDocument(SnapshotVersion.min()));
        t.push(this.vr.addEntry(e, i, n2));
      }
    }), r.forEach((n2) => {
      t.push(this.vr.indexManager.addToCollectionParentIndex(e, n2));
    }), t.push(this.vr.updateMetadata(e, n)), PersistencePromise.waitFor(t);
  }
  getFromCache(e, t) {
    return this.vr.wr(e, t).next((e2) => (this.Cr.set(t, {
      size: e2.size,
      readTime: e2.document.readTime
    }), e2.document));
  }
  getAllFromCache(e, t) {
    return this.vr.Sr(e, t).next(({ documents: e2, Dr: t2 }) => (
...
var __PRIVATE_MemoryMutationQueue = class {
  ...
  getHighestUnacknowledgedBatchId() {
    return PersistencePromise.resolve(0 === this.mutationQueue.length ? W : this.Jn - 1);
  }
  getAllMutationBatches(e) {
...
var __PRIVATE_MemoryLruDelegate = class ___PRIVATE_MemoryLruDelegate {
  ...
  Ar(e, t) {
    return PersistencePromise.forEach(this.Ri, (n, r) => this.gr(e, n, r).next((e2) => e2 ? PersistencePromise.resolve() : t(r)));
  }
  removeTargets(e, t, n) {
  ...
  removeOrphanedDocuments(e, t) {
    ...
    return r.Xr(e, (r2) => this.gr(e, r2, t).next((e2) => {
  ...
  gr(e, t, n) {
    return PersistencePromise.or([() => this.persistence.Pi(e, t), () => this.persistence.getTargetCache().containsKey(e, t), () => {
...
var __PRIVATE_SchemaConverter = class {
  ...
  q(e, t, n, r) {
    ...
    n < 1 && r >= 1 && (!function __PRIVATE_createPrimaryClientStore(e2) {
      ...
      const t2 = e2.createObjectStore(Z, {
        keyPath: X,
        autoIncrement: true
      });
      t2.createIndex(ee, te, {
        unique: true
    ...
    (0 !== n && (!function __PRIVATE_dropQueryCache(e2) {
      e2.deleteObjectStore(Ie), e2.deleteObjectStore(he), e2.deleteObjectStore(Ve);
    ...
    (s = s.next(() => function __PRIVATE_upgradeMutationBatchSchemaAndMigrateData(e2, t2) {
      ...
      return n2.J().next((n3) => {
        ...
        const r2 = t2.store(Z), i2 = n3.map((e3) => r2.put(e3));
    ...
    })), n < 5 && r >= 5 && (s = s.next(() => this.Vi(i))), n < 6 && r >= 6 && (s = s.next(() => (function __PRIVATE_createDocumentGlobalStore(e2) {
      e2.createObjectStore(ce);
    }(e), this.mi(i)))), n < 7 && r >= 7 && (s = s.next(() => this.fi(i))), n < 8 && r >= 8 && (s = s.next(() => this.gi(e, i))), n < 9 && r >= 9 && (s = s.next(() => {
    ...
    })), n < 10 && r >= 10 && (s = s.next(() => this.pi(i))), n < 11 && r >= 11 && (s = s.next(() => {
      !function __PRIVATE_createBundlesStore(e2) {
        e2.createObjectStore(ye, {
          keyPath: we
        });
      ...
      !function __PRIVATE_createDocumentOverlayStore(e2) {
        const t2 = e2.createObjectStore(Qe, {
          keyPath: $e
        });
        t2.createIndex(Ue, Ke, {
          unique: false
        }), t2.createIndex(We, Ge, {
          unique: false
        });
      }(e);
    })), n < 13 && r >= 13 && (s = s.next(() => function __PRIVATE_createRemoteDocumentCache(e2) {
      const t2 = e2.createObjectStore(ie, {
        keyPath: se
      });
      t2.createIndex(oe, _e), t2.createIndex(ae, ue);
    }(e)).next(() => this.yi(e, i)).next(() => e.deleteObjectStore(z))), n < 14 && r >= 14 && (s = s.next(() => this.wi(e, i))), n < 15 && r >= 15 && (s = s.next(() => function __PRIVATE_createFieldIndex(e2) {
      const t2 = e2.createObjectStore(De, {
        keyPath: ve,
        autoIncrement: true
      });
      t2.createIndex(Ce, Fe, {
        unique: false
      });
      const n2 = e2.createObjectStore(Me, {
        keyPath: xe
      });
      n2.createIndex(Oe, Ne, {
        unique: false
      });
      const r2 = e2.createObjectStore(Be, {
        keyPath: Le
      });
      r2.createIndex(ke, qe, {
        unique: false
      });
    ...
    (s = s.next(() => {
      ...
      t.objectStore(Be).clear();
    })), n < 17 && r >= 17 && (s = s.next(() => {
      !function __PRIVATE_createGlobalsStore(e2) {
        e2.createObjectStore(ze, {
          keyPath: je
        });
      }(e);
    })), s;
  ...
  mi(e) {
    let t = 0;
    return e.store(z).te((e2, n) => {
      t += __PRIVATE_dbDocumentSize(n);
    }).next(() => {
      const n = {
        byteSize: t
      };
      return e.store(ce).put(le, n);
    });
  }
  Vi(e) {
    const t = e.store(J), n = e.store(Z);
    return t.J().next((t2) => PersistencePromise.forEach(t2, (t3) => {
      const r = IDBKeyRange.bound([t3.userId, W], [t3.userId, t3.lastAcknowledgedBatchId]);
      return n.J(ee, r).next((n2) => PersistencePromise.forEach(n2, (n3) => {
   ...
   */
    ...
    const t = e.store(Ie), n = e.store(z);
    return e.store(Ve).get(Re).next((e2) => {
      const r = [];
      return n.te((n2, i) => {
  ...
  gi(e, t) {
    ...
    const n = t.store(me), r = new __PRIVATE_MemoryCollectionParentIndex(), addEntry = (e2) => {
      if (r.add(e2)) {
  ...
  yi(e, t) {
    const n = t.store(z), r = [];
    return n.te((e2, n2) => {
      const i = t.store(ie), s = function __PRIVATE_extractKey(e3) {
  ...
  wi(e, t) {
    const n = t.store(Z), r = __PRIVATE_newIndexedDbRemoteDocumentCache(this.serializer), i = new __PRIVATE_MemoryPersistence(__PRIVATE_MemoryEagerDelegate.Ei, this.serializer.wt);
    return n.J().next((e2) => {
      const n2 = /* @__PURE__ */ new Map();
...
function __PRIVATE_createQueryCache(e) {
  ...
  e.createObjectStore(he, {
    ...
    unique: true
  }), e.createObjectStore(Ve);
}
var xt = "IndexedDbPersistence";
var Ot = 18e5;
var Nt = 5e3;
var Bt = "Failed to obtain exclusive access to the persistence layer. To allow shared access, multi-tab synchronization has to be enabled in all tabs. If you are using `experimentalForceOwningTab:true`, make sure that only one tab has persistence enabled at any given time.";
var Lt = "main";
var __PRIVATE_IndexedDbPersistence = class ___PRIVATE_IndexedDbPersistence {
  constructor(e, t, n, r, i, s, o, _, a, u, c = 17) {
    if (this.allowTabSynchronization = e, this.persistenceKey = t, this.clientId = n, this.bi = i, this.window = s, this.document = o, this.Si = a, this.Di = u, this.Ci = c, this.si = null, this.oi = false, this.isPrimary = false, this.networkEnabled = true, /** Our window.unload handler, if registered. */
    this.Fi = null, this.inForeground = false, /** Our 'visibilitychange' listener if registered. */
    this.Mi = null, /** The client metadata refresh task. */
    this.xi = null, /** The last time we garbage collected the client metadata object store. */
    this.Oi = Number.NEGATIVE_INFINITY, /** A listener to notify on primary state changes. */
    this.Ni = (e2) => Promise.resolve(), !___PRIVATE_IndexedDbPersistence.C())
      throw new FirestoreError(O.UNIMPLEMENTED, "This platform is either missing IndexedDB or is known to have an incomplete implementation. Offline persistence has been disabled.");
    this.referenceDelegate = new __PRIVATE_IndexedDbLruDelegateImpl(this, r), this.Bi = t + Lt, this.serializer = new __PRIVATE_LocalSerializer(_), this.Li = new __PRIVATE_SimpleDb(this.Bi, this.Ci, new __PRIVATE_SchemaConverter(this.serializer)), this._i = new __PRIVATE_IndexedDbGlobalsCache(), this.ai = new __PRIVATE_IndexedDbTargetCache(this.referenceDelegate, this.serializer), this.remoteDocumentCache = __PRIVATE_newIndexedDbRemoteDocumentCache(this.serializer), this.ci = new __PRIVATE_IndexedDbBundleCache(), this.window && this.window.localStorage ? this.ki = this.window.localStorage : (this.ki = null, false === u && __PRIVATE_logError(xt, "LocalStorage is unavailable. As a result, persistence may not work reliably. In particular enablePersistence() could fail immediately after refreshing the page."));
  }
  /**
  ...
  start() {
    return this.qi().then(() => {
      if (!this.isPrimary && !this.allowTabSynchronization)
        throw new FirestoreError(O.FAILED_PRECONDITION, Bt);
      return this.Qi(), this.$i(), this.Ui(), this.runTransaction("getHighestListenSequenceNumber", "readonly", (e) => this.ai.getHighestSequenceNumber(e));
    }).then((e) => {
      this.si = new __PRIVATE_ListenSequence(e, this.Si);
    }).then(() => {
      this.oi = true;
    }).catch((e) => (this.Li && this.Li.close(), Promise.reject(e)));
  }
  /**
    ...
    return this.runTransaction("updateClientMetadataAndTryBecomePrimary", "readwrite", (e) => __PRIVATE_clientMetadataStore(e).put({
      ...
      if (this.isPrimary)
        return this.Wi(e).next((e2) => {
          e2 || (this.isPrimary = false, this.bi.enqueueRetryable(() => this.Ni(false)));
        });
    }).next(() => this.Gi(e)).next((t) => this.isPrimary && !t ? this.zi(e).next(() => false) : !!t && this.ji(e).next(() => true))).catch((e) => {
      if (__PRIVATE_isIndexedDbTransactionError(e))
        return __PRIVATE_logDebug(xt, "Failed to extend owner lease: ", e), this.isPrimary;
      if (!this.allowTabSynchronization)
        throw e;
      return __PRIVATE_logDebug(xt, "Releasing owner lease after error during lease refresh", e), /* isPrimary= */
      false;
    }).then((e) => {
      this.isPrimary !== e && this.bi.enqueueRetryable(() => this.Ni(e)), this.isPrimary = e;
    });
  }
  Wi(e) {
    return __PRIVATE_primaryClientStore(e).get(H).next((e2) => PersistencePromise.resolve(this.Hi(e2)));
  }
  Ji(e) {
   ...
   */
  async Yi() {
    if (this.isPrimary && !this.Zi(this.Oi, Ot)) {
      this.Oi = Date.now();
      const e = await this.runTransaction("maybeGarbageCollectMultiClientState", "readwrite-primary", (e2) => {
        const t = __PRIVATE_getStore(e2, ge);
        return t.J().next((e3) => {
          const n = this.Xi(e3, Ot), r = e3.filter((e4) => -1 === n.indexOf(e4));
  ...
  Gi(e) {
    ...
    return __PRIVATE_primaryClientStore(e).get(H).next((t) => {
      if (null !== t && this.Zi(t.leaseTimestampMs, Nt) && !this.ts(t.ownerId)) {
      ...
      return !(!this.networkEnabled || !this.inForeground) || __PRIVATE_clientMetadataStore(e).J().next((e2) => void 0 === this.Xi(e2, Nt).find((e3) => {
    ...
    }).next((e2) => (this.isPrimary !== e2 && __PRIVATE_logDebug(xt, `Client ${e2 ? "is" : "is not"} eligible for a primary lease.`), e2));
  }
  async shutdown() {
    this.oi = false, this.ns(), this.xi && (this.xi.cancel(), this.xi = null), this.rs(), this.ss(), // Use `SimpleDb.runTransaction` directly to avoid failing if another tab
    // has obtained the primary lease.
    await this.Li.runTransaction("shutdown", "readwrite", [j, ge], (e) => {
      const t = new __PRIVATE_IndexedDbTransaction(e, __PRIVATE_ListenSequence.le);
      return this.zi(t).next(() => this.Ji(t));
    }), this.Li.close(), // Remove the entry marking the client as zombied from LocalStorage since
   ...
   */
  Xi(e, t) {
    return e.filter((e2) => this.Zi(e2.updateTimeMs, t) && !this.ts(e2.clientId));
  }
  /**
   ...
   */
  us() {
    return this.runTransaction("getActiveClients", "readonly", (e) => __PRIVATE_clientMetadataStore(e).J().next((e2) => this.Xi(e2, Ot).map((e3) => e3.clientId)));
  }
  get started() {
    return this.oi;
  }
  ...
  runTransaction(e, t, n) {
    __PRIVATE_logDebug(xt, "Starting transaction:", e);
    const r = "readonly" === t ? "readonly" : "readwrite", i = (
      /** Returns the object stores for the provided schema. */
      function __PRIVATE_getObjectStores(e2) {
        return 17 === e2 ? tt : 16 === e2 ? et : 15 === e2 ? Xe : 14 === e2 ? Ze : 13 === e2 ? Ye : 12 === e2 ? Je : 11 === e2 ? He : void fail(60245);
      }(this.Ci)
    );
    let s;
    return this.Li.runTransaction(e, r, i, (r2) => (s = new __PRIVATE_IndexedDbTransaction(r2, this.si ? this.si.next() : __PRIVATE_ListenSequence.le), "readwrite-primary" === t ? this.Wi(s).next((e2) => !!e2 || this.Gi(s)).next((t2) => {
      if (!t2)
        throw __PRIVATE_logError(`Failed to obtain primary lease for action '${e}'.`), this.isPrimary = false, this.bi.enqueueRetryable(() => this.Ni(false)), new FirestoreError(O.FAILED_PRECONDITION, Q);
      return n(s);
    }).next((e2) => this.ji(s).next(() => e2)) : this.cs(s).next(() => n(s)))).then((e2) => (s.raiseOnCommittedEvent(), e2));
  }
  /**
  ...
  cs(e) {
    return __PRIVATE_primaryClientStore(e).get(H).next((e2) => {
      if (null !== e2 && this.Zi(e2.leaseTimestampMs, Nt) && !this.ts(e2.ownerId) && !this.Hi(e2) && !(this.Di || this.allowTabSynchronization && e2.allowTabSynchronization))
        throw new FirestoreError(O.FAILED_PRECONDITION, Bt);
    });
  }
  /**
  ...
  zi(e) {
    ...
    return t.get(H).next((e2) => this.Hi(e2) ? (__PRIVATE_logDebug(xt, "Releasing primary lease."), t.delete(H)) : PersistencePromise.resolve());
  }
  ...
  Qi() {
    null !== this.document && "function" == typeof this.document.addEventListener && (this.Mi = () => {
      this.bi.enqueueAndForget(() => (this.inForeground = "visible" === this.document.visibilityState, this.qi()));
    }, this.document.addEventListener("visibilitychange", this.Mi), this.inForeground = "visible" === this.document.visibilityState);
  }
  rs() {
    this.Mi && (this.document.removeEventListener("visibilitychange", this.Mi), this.Mi = null);
  }
  /**
  ...
  $i() {
    var e;
    "function" == typeof (null === (e = this.window) || void 0 === e ? void 0 : e.addEventListener) && (this.Fi = () => {
      this.ns();
  ...
  ss() {
    this.Fi && (this.window.removeEventListener("pagehide", this.Fi), this.Fi = null);
  }
  /**
  ...
  ts(e) {
    var t;
    try {
      const n = null !== (null === (t = this.ki) || void 0 === t ? void 0 : t.getItem(this.es(e)));
      return __PRIVATE_logDebug(xt, `Client '${e}' ${n ? "is" : "is not"} zombied in LocalStorage`), n;
    } catch (e2) {
      return __PRIVATE_logError(xt, "Failed to get zombied client id.", e2), false;
    }
  }
  /**
...
var __PRIVATE_QueryEngine = class {
  ...
  getDocumentsMatchingQuery(e, t, n, r) {
    ...
    return this.Rs(e, t).next((e2) => {
      ...
      if (!i.result)
        return this.Vs(e, t, r, n).next((e2) => {
          i.result = e2;
        });
    }).next(() => {
      if (i.result)
      ...
      return this.fs(e, t, n2).next((r2) => {
        if (i.result = r2, this.Is)
          return this.gs(e, t, n2, r2.size);
      });
    }).next(() => i.result);
  }
  gs(e, t, n, r) {
  ...
  Vs(e, t, n, r) {
    return __PRIVATE_queryMatchesAllDocuments(t) || r.isEqual(SnapshotVersion.min()) ? PersistencePromise.resolve(null) : this.As.getDocuments(e, n).next((i) => {
      const s = this.ps(t, i);
      return this.ys(t, s, n, r) ? PersistencePromise.resolve(null) : (__PRIVATE_getLogLevel() <= LogLevel.DEBUG && __PRIVATE_logDebug("QueryEngine", "Re-using previous result from %s to execute query: %s", r.toString(), __PRIVATE_stringifyQuery(t)), this.ws(e, s, t, __PRIVATE_newIndexOffsetSuccessorFromReadTime(r, q)).next((e2) => e2));
    });
  }
...
function __PRIVATE_localStoreGetLastRemoteSnapshotVersion(e) {
  const t = __PRIVATE_debugCast(e);
  return t.persistence.runTransaction("Get last remote snapshot version", "readonly", (e2) => t.ai.getLastRemoteSnapshotVersion(e2));
}
function __PRIVATE_localStoreApplyRemoteEventToLocalCache(e, t) {
  ...
  return n.persistence.runTransaction("Apply remote event", "readwrite-primary", (e2) => {
    ...
    t.targetChanges.forEach((s2, _2) => {
      ...
      if (!a2)
        return;
      o.push(n.ai.removeMatchingKeys(e2, s2.removedDocuments, _2).next(() => n.ai.addMatchingKeys(e2, s2.addedDocuments, _2)));
      ...
      */
      function __PRIVATE_shouldPersistTargetData(e3, t2, n2) {
        ...
        return i2 > 0;
      }(a2, u, s2) && o.push(n.ai.updateTargetData(e2, u));
    });
    ...
    o.push(__PRIVATE_populateDocumentChangeBuffer(e2, s, t.documentUpdates).next((e3) => {
      ...
      const t2 = n.ai.getLastRemoteSnapshotVersion(e2).next((t3) => n.ai.setTargetsMetadata(e2, e2.currentSequenceNumber, r));
      o.push(t2);
    }
...
function __PRIVATE_populateDocumentChangeBuffer(e, t, n) {
  ...
  return n.forEach((e2) => r = r.add(e2)), t.getEntries(e, r).next((e2) => {
    ...
    return n.forEach((n2, s) => {
      ...
      ) : !o.isValidDocument() || s.version.compareTo(o.version) > 0 || 0 === s.version.compareTo(o.version) && o.hasPendingWrites ? (t.addEntry(s), r2 = r2.insert(n2, s)) : __PRIVATE_logDebug(kt, "Ignoring outdated watch update for ", n2, ". Current version:", o.version, " Watch version:", s.version);
    }), {
      xs: r2,
      Os: i
    };
...
function __PRIVATE_localStoreGetNextMutationBatch(e, t) {
  const n = __PRIVATE_debugCast(e);
  return n.persistence.runTransaction("Get next mutation batch", "readonly", (e2) => (void 0 === t && (t = W), n.mutationQueue.getNextMutationBatchAfterBatchId(e2, t)));
}
function __PRIVATE_localStoreAllocateTarget(e, t) {
  const n = __PRIVATE_debugCast(e);
  return n.persistence.runTransaction("Allocate target", "readwrite", (e2) => {
    let r;
    return n.ai.getTargetData(e2, t).next((i) => i ? (
      // This target has been listened to previously, so reuse the
      // previous targetID.
      // TODO(mcg): freshen last accessed date?
      (r = i, PersistencePromise.resolve(r))
    ) : n.ai.allocateTargetId(e2).next((i2) => (r = new TargetData(t, i2, "TargetPurposeListen", e2.currentSequenceNumber), n.ai.addTargetData(e2, r).next(() => r))));
  }).then((e2) => {
    const r = n.Ss.get(e2.targetId);
    return (null === r || e2.snapshotVersion.compareTo(r.snapshotVersion) > 0) && (n.Ss = n.Ss.insert(e2.targetId, e2), n.Ds.set(t, e2.targetId)), e2;
  });
}
async function __PRIVATE_localStoreReleaseTarget(e, t, n) {
  ...
  try {
    ...
    if (!__PRIVATE_isIndexedDbTransactionError(e2))
      throw e2;
    __PRIVATE_logDebug(kt, `Failed to update sequence numbers for target ${t}: ${e2}`);
  }
  r.Ss = r.Ss.remove(t), r.Ds.delete(i.target);
}
function __PRIVATE_localStoreExecuteQuery(e, t, n) {
  ...
  return r.persistence.runTransaction(
    ...
    (e2) => function __PRIVATE_localStoreGetTargetData(e3, t2, n2) {
      const r2 = __PRIVATE_debugCast(e3), i2 = r2.Ds.get(n2);
      return void 0 !== i2 ? PersistencePromise.resolve(r2.Ss.get(i2)) : r2.ai.getTargetData(t2, n2);
    }(r, e2, __PRIVATE_queryToTarget(t)).next((t2) => {
      if (t2)
        return i = t2.lastLimboFreeSnapshotVersion, r.ai.getMatchingKeysForTargetId(e2, t2.targetId).next((e3) => {
          s = e3;
        });
    }).next(() => r.bs.getDocumentsMatchingQuery(e2, t, n ? i : SnapshotVersion.min(), n ? s : __PRIVATE_documentKeySet())).next((e3) => (__PRIVATE_setMaxReadTime(r, __PRIVATE_queryCollectionGroup(t), e3), {
      documents: e3,
      Ns: s
    }))
  );
...
function __PRIVATE_localStoreGetCachedTarget(e, t) {
  const n = __PRIVATE_debugCast(e), r = __PRIVATE_debugCast(n.ai), i = n.Ss.get(t);
...
function __PRIVATE_localStoreGetNewDocumentChanges(e, t) {
  const n = __PRIVATE_debugCast(e), r = n.vs.get(t) || SnapshotVersion.min();
...
function __PRIVATE_setMaxReadTime(e, t, n) {
  let r = e.vs.get(t) || SnapshotVersion.min();
  n.forEach((e2, t2) => {
...
async function __PRIVATE_localStoreApplyBundledDocuments(e, t, n, r) {
  ...
  for (const e2 of n) {
    const n2 = t.Bs(e2.metadata.name);
  ...
  return i.persistence.runTransaction("Apply bundle documents", "readwrite", (e2) => __PRIVATE_populateDocumentChangeBuffer(e2, _, o).next((t2) => (_.apply(e2), t2)).next((t2) => i.ai.removeMatchingKeysForTargetId(e2, a.targetId).next(() => i.ai.addMatchingKeys(e2, s, a.targetId)).next(() => i.localDocuments.getLocalViewOfDocuments(e2, t2.xs, t2.Os)).next(() => t2.xs)));
}
async function __PRIVATE_localStoreSaveNamedQuery(e, t, n = __PRIVATE_documentKeySet()) {
  ...
  return i.persistence.runTransaction("Save named query", "readwrite", (e2) => {
    ...
    if (r.snapshotVersion.compareTo(s) >= 0)
      return i.ci.saveNamedQuery(e2, t);
    const o = r.withResumeToken(ByteString.EMPTY_BYTE_STRING, s);
    return i.Ss = i.Ss.insert(o.targetId, o), i.ai.updateTargetData(e2, o).next(() => i.ai.removeMatchingKeysForTargetId(e2, r.targetId)).next(() => i.ai.addMatchingKeys(e2, n, r.targetId)).next(() => i.ci.saveNamedQuery(e2, t));
  });
}
var Qt = "firestore_clients";
function createWebStorageClientStateKey(e, t) {
  return `${Qt}_${e}_${t}`;
}
var $t = "firestore_mutations";
function createWebStorageMutationBatchKey(e, t, n) {
  let r = `${$t}_${e}_${n}`;
  return t.isAuthenticated() && (r += `_${t.uid}`), r;
}
var Ut = "firestore_targets";
function createWebStorageQueryTargetMetadataKey(e, t) {
  return `${Ut}_${e}_${t}`;
}
var Kt = "SharedClientState";
var __PRIVATE_MutationMetadata = class ___PRIVATE_MutationMetadata {
  ...
  static qs(e, t, n) {
    ...
    return s && r.error && (s = "string" == typeof r.error.message && "string" == typeof r.error.code, s && (i = new FirestoreError(r.error.code, r.error.message))), s ? new ___PRIVATE_MutationMetadata(e, t, r.state, i) : (__PRIVATE_logError(Kt, `Failed to parse mutation state for ID '${t}': ${n}`), null);
  }
  Qs() {
    const e = {
      state: this.state,
...
var __PRIVATE_QueryTargetMetadata = class ___PRIVATE_QueryTargetMetadata {
  ...
  static qs(e, t) {
    ...
    return i && n.error && (i = "string" == typeof n.error.message && "string" == typeof n.error.code, i && (r = new FirestoreError(n.error.code, n.error.message))), i ? new ___PRIVATE_QueryTargetMetadata(e, n.state, r) : (__PRIVATE_logError(Kt, `Failed to parse target state for ID '${e}': ${t}`), null);
  }
  Qs() {
    const e = {
      state: this.state,
...
var __PRIVATE_RemoteClientState = class ___PRIVATE_RemoteClientState {
  ...
  static qs(e, t) {
    ...
    return r ? new ___PRIVATE_RemoteClientState(e, i) : (__PRIVATE_logError(Kt, `Failed to parse client data for instance '${e}': ${t}`), null);
  }
};
var __PRIVATE_SharedOnlineState = class ___PRIVATE_SharedOnlineState {
  ...
  static qs(e) {
    ...
    return "object" == typeof t && -1 !== ["Unknown", "Online", "Offline"].indexOf(t.onlineState) && "string" == typeof t.clientId ? new ___PRIVATE_SharedOnlineState(t.clientId, t.onlineState) : (__PRIVATE_logError(Kt, `Failed to parse online state: ${e}`), null);
  }
};
...
var __PRIVATE_WebStorageSharedClientState = class {
  constructor(e, t, n, r, i) {
    this.window = e, this.bi = t, this.persistenceKey = n, this.Ks = r, this.syncEngine = null, this.onlineStateHandler = null, this.sequenceNumberHandler = null, this.Ws = this.Gs.bind(this), this.zs = new SortedMap(__PRIVATE_primitiveComparator), this.started = false, /**
     * Captures WebStorage events that occur before `start()` is called. These
     * events are replayed once `WebStorageSharedClientState` is started.
     */
    this.js = [];
    const s = n.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    this.storage = this.window.localStorage, this.currentUser = i, this.Hs = createWebStorageClientStateKey(this.persistenceKey, this.Ks), this.Js = /** Assembles the key for the current sequence number. */
    function createWebStorageSequenceNumberKey(e2) {
      return `firestore_sequence_number_${e2}`;
    }(this.persistenceKey), this.zs = this.zs.insert(this.Ks, new __PRIVATE_LocalClientState()), this.Ys = new RegExp(`^${Qt}_${s}_([^_]*)$`), this.Zs = new RegExp(`^${$t}_${s}_(\\d+)(?:_(.*))?$`), this.Xs = new RegExp(`^${Ut}_${s}_(\\d+)$`), this.eo = /** Assembles the key for the online state of the primary tab. */
    function createWebStorageOnlineStateKey(e2) {
      return `firestore_online_state_${e2}`;
    }(this.persistenceKey), this.no = function createBundleLoadedKey(e2) {
  ...
  async start() {
    const e = await this.syncEngine.us();
    for (const t2 of e) {
      if (t2 === this.Ks)
        continue;
      const e2 = this.getItem(createWebStorageClientStateKey(this.persistenceKey, t2));
      if (e2) {
        const n = __PRIVATE_RemoteClientState.qs(t2, e2);
        n && (this.zs = this.zs.insert(n.clientId, n));
      }
    }
    this.ro();
    const t = this.storage.getItem(this.eo);
    if (t) {
      const e2 = this.io(t);
      e2 && this.so(e2);
    }
    for (const e2 of this.js)
      this.Gs(e2);
  ...
  updateMutationState(e, t, n) {
    this._o(e, t, n), // Once a final mutation result is observed by other clients, they no longer
  ...
  addLocalQueryTarget(e, t = true) {
    ...
    if (this.isActiveQueryTarget(e)) {
      ...
      if (t2) {
        const r = __PRIVATE_QueryTargetMetadata.qs(e, t2);
        r && (n = r.state);
      }
    }
    return t && this.uo.$s(e), this.ro(), n;
  }
  removeLocalQueryTarget(e) {
    this.uo.Us(e), this.ro();
  }
  ...
  shutdown() {
    this.started && (this.window.removeEventListener("storage", this.Ws), this.removeItem(this.Hs), this.started = false);
  }
  getItem(e) {
  ...
  Gs(e) {
    ...
    if (t.storageArea === this.storage) {
      if (__PRIVATE_logDebug(Kt, "EVENT", t.key, t.newValue), t.key === this.Hs)
      ...
      this.bi.enqueueRetryable(async () => {
        if (this.started) {
          if (null !== t.key) {
            if (this.Ys.test(t.key)) {
              if (null == t.newValue) {
                const e2 = this.Po(t.key);
                return this.To(e2, null);
              }
              {
                const e2 = this.Io(t.key, t.newValue);
                if (e2)
                  return this.To(e2.clientId, e2);
              }
            } else if (this.Zs.test(t.key)) {
              if (null !== t.newValue) {
                const e2 = this.Eo(t.key, t.newValue);
                if (e2)
                  return this.Ao(e2);
              }
            } else if (this.Xs.test(t.key)) {
              if (null !== t.newValue) {
              ...
              if (null !== t.newValue) {
                const e2 = this.io(t.newValue);
                if (e2)
                  return this.so(e2);
              }
            } else if (t.key === this.Js) {
              const e2 = function __PRIVATE_fromWebStorageSequenceNumber(e3) {
                ...
                if (null != e3)
                  try {
                    ...
                    __PRIVATE_logError(Kt, "Failed to read sequence number from WebStorage", e4);
                  }
                return t2;
              }(t.newValue);
              e2 !== __PRIVATE_ListenSequence.le && this.sequenceNumberHandler(e2);
            } else if (t.key === this.no) {
              const e2 = this.fo(t.newValue);
              await Promise.all(e2.map((e3) => this.syncEngine.po(e3)));
            }
          }
        } else
          this.js.push(t);
  ...
  Io(e, t) {
    const n = this.Po(e);
    return __PRIVATE_RemoteClientState.qs(n, t);
  }
  /**
   ...
   */
  Eo(e, t) {
    const n = this.Zs.exec(e), r = Number(n[1]), i = void 0 !== n[2] ? n[2] : null;
    return __PRIVATE_MutationMetadata.qs(new User(i), r, t);
  }
  /**
   ...
   */
  Ro(e, t) {
    const n = this.Xs.exec(e), r = Number(n[1]);
    return __PRIVATE_QueryTargetMetadata.qs(r, t);
  }
  /**
   ...
   */
  io(e) {
    return __PRIVATE_SharedOnlineState.qs(e);
  }
  fo(e) {
    return JSON.parse(e);
  }
  ...
  To(e, t) {
    const n = t ? this.zs.insert(e, t) : this.zs.remove(e), r = this.oo(this.zs), i = this.oo(n), s = [], o = [];
    return i.forEach((e2) => {
      r.has(e2) || s.push(e2);
    }), r.forEach((e2) => {
      i.has(e2) || o.push(e2);
    }), this.syncEngine.bo(s, o).then(() => {
      this.zs = n;
    });
  }
  so(e) {
...
var __PRIVATE_BrowserConnectivityMonitor = class {
  ...
  Fo() {
    __PRIVATE_logDebug(Wt, "Network connectivity changed: AVAILABLE");
  ...
  xo() {
    __PRIVATE_logDebug(Wt, "Network connectivity changed: UNAVAILABLE");
    for (const e of this.Oo)
      e(
        1
...
function __PRIVATE_generateUniqueDebugId() {
  return null === Gt ? Gt = function __PRIVATE_generateInitialUniqueDebugId() {
    return 268435456 + Math.round(2147483648 * Math.random());
  }() : Gt++, "0x" + Gt.toString(16);
}
var zt = "RestConnection";
var jt = {
...
var __PRIVATE_RestConnection = class {
  ...
  constructor(e) {
    ...
    this.Lo = t + "://" + e.host, this.ko = `projects/${n}/databases/${r}`, this.qo = this.databaseId.database === _t ? `project_id=${n}` : `project_id=${n}&database_id=${r}`;
  }
  Qo(e, t, n, r, i) {
    const s = __PRIVATE_generateUniqueDebugId(), o = this.$o(e, t.toUriEncodedString());
    __PRIVATE_logDebug(zt, `Sending RPC '${e}' ${s}:`, o, n);
    const _ = {
    ...
    return this.Ko(e, o, _, n, u).then((t2) => (__PRIVATE_logDebug(zt, `Received RPC '${e}' ${s}: `, t2), t2), (t2) => {
      throw __PRIVATE_logWarn(zt, `RPC '${e}' ${s} failed with error: `, t2, "url: ", o, "request:", n), t2;
    });
  }
  Wo(e, t, n, r, i, s) {
        ...
        try {
          switch (_.getLastErrorCode()) {
            case ErrorCode.NO_ERROR:
              ...
              __PRIVATE_logDebug(Ht, `XHR for RPC '${e}' ${s} received:`, JSON.stringify(t2)), i2(t2);
              break;
            case ErrorCode.TIMEOUT:
              __PRIVATE_logDebug(Ht, `RPC '${e}' ${s} timed out`), o(new FirestoreError(O.DEADLINE_EXCEEDED, "Request time out"));
              break;
            case ErrorCode.HTTP_ERROR:
              const n2 = _.getStatus();
              if (__PRIVATE_logDebug(Ht, `RPC '${e}' ${s} failed with status:`, n2, "response text:", _.getResponseText()), n2 > 0) {
                ...
                if (t3 && t3.status && t3.message) {
                  const e3 = function __PRIVATE_mapCodeFromHttpResponseErrorStatus(e4) {
                    ...
                    return Object.values(O).indexOf(t4) >= 0 ? t4 : O.UNKNOWN;
                  }(t3.status);
                  o(new FirestoreError(e3, t3.message));
                } else
                  o(new FirestoreError(O.UNKNOWN, "Server responded with status " + _.getStatus()));
              } else
                o(new FirestoreError(O.UNAVAILABLE, "Connection failed."));
              break;
            default:
              fail(9055, {
                s_: e,
          ...
          __PRIVATE_logDebug(Ht, `RPC '${e}' ${s} completed.`);
        }
      });
      const a = JSON.stringify(r);
      __PRIVATE_logDebug(Ht, `RPC '${e}' ${s} sending request:`, r), _.send(t, "POST", a, n, 15);
    });
  }
  a_(e, t, n) {
    ...
    const u = i.join("");
    __PRIVATE_logDebug(Ht, `Creating RPC '${e}' stream ${r}: ${u}`, _);
    const c = s.createWebChannel(u, _);
    let l = false, h = false;
    const P = new __PRIVATE_StreamBridge({
      Go: (t2) => {
        h ? __PRIVATE_logDebug(Ht, `Not sending because RPC '${e}' stream ${r} is closed:`, t2) : (l || (__PRIVATE_logDebug(Ht, `Opening RPC '${e}' stream ${r} transport.`), c.open(), l = true), __PRIVATE_logDebug(Ht, `RPC '${e}' stream ${r} sending:`, t2), c.send(t2));
      },
      zo: () => c.close()
    ...
    return __PRIVATE_unguardedEventListen(c, WebChannel.EventType.OPEN, () => {
      h || (__PRIVATE_logDebug(Ht, `RPC '${e}' stream ${r} transport opened.`), P.t_());
    }), __PRIVATE_unguardedEventListen(c, WebChannel.EventType.CLOSE, () => {
      h || (h = true, __PRIVATE_logDebug(Ht, `RPC '${e}' stream ${r} transport closed`), P.r_());
    }), __PRIVATE_unguardedEventListen(c, WebChannel.EventType.ERROR, (t2) => {
      h || (h = true, __PRIVATE_logWarn(Ht, `RPC '${e}' stream ${r} transport errored. Name:`, t2.name, "Message:", t2.message), P.r_(new FirestoreError(O.UNAVAILABLE, "The operation could not be completed")));
      ...
      if (!h) {
        ...
        if (o2) {
          __PRIVATE_logDebug(Ht, `RPC '${e}' stream ${r} received error:`, o2);
          const t3 = o2.status;
          let n3 = (
            /**
          ...
          void 0 === n3 && (n3 = O.INTERNAL, i3 = "Unknown error status: " + t3 + " with message " + o2.message), // Mark closed so no further events are propagated
          h = true, P.r_(new FirestoreError(n3, i3)), c.close();
        } else
          __PRIVATE_logDebug(Ht, `RPC '${e}' stream ${r} received:`, i2), P.i_(i2);
      }
    }), __PRIVATE_unguardedEventListen(o, Event.STAT_EVENT, (t2) => {
      t2.stat === Stat.PROXY ? __PRIVATE_logDebug(Ht, `RPC '${e}' stream ${r} detected buffering proxy`) : t2.stat === Stat.NOPROXY && __PRIVATE_logDebug(Ht, `RPC '${e}' stream ${r} detected no buffering proxy`);
    }), setTimeout(() => {
      P.n_();
    }, 0), P;
  }
...
var __PRIVATE_PersistentStream = class {
  constructor(e, t, n, r, i, s, o, _) {
    this.bi = e, this.R_ = n, this.V_ = r, this.connection = i, this.authCredentialsProvider = s, this.appCheckCredentialsProvider = o, this.listener = _, this.state = 0, /**
     * A close count that's incremented every time the stream is closed; used by
     * getCloseGuardedDispatcher() to invalidate callbacks that happen after
     * close.
     */
    this.m_ = 0, this.f_ = null, this.g_ = null, this.stream = null, /**
     * Count of response messages received.
     */
    this.p_ = 0, this.y_ = new __PRIVATE_ExponentialBackoff(e, t);
  }
  /**
   ...
   */
  start() {
    this.p_ = 0, 4 !== this.state ? this.auth() : this.S_();
  }
  /**
  ...
  x_() {
    this.g_ && (this.g_.cancel(), this.g_ = null);
  }
  /**
   ...
   */
  async close(e, t) {
    this.M_(), this.x_(), this.y_.cancel(), // Invalidates any stream-related callbacks (e.g. from auth or the
    // underlying stream), guaranteeing they won't execute.
    this.m_++, 4 !== e ? (
      // If this is an intentional close ensure we don't delay our next connection attempt.
      this.y_.reset()
    ) : t && t.code === O.RESOURCE_EXHAUSTED ? (
      // Log the error. (Probably either 'quota exceeded' or 'max queue length reached'.)
      (__PRIVATE_logError(t.toString()), __PRIVATE_logError("Using maximum backoff delay to prevent overloading the backend."), this.y_.I_())
    ) : t && t.code === O.UNAUTHENTICATED && 3 !== this.state && // "unauthenticated" error means the token was rejected. This should rarely
  ...
  auth() {
    this.state = 1;
    const e = this.N_(this.m_), t = this.m_;
    Promise.all([this.authCredentialsProvider.getToken(), this.appCheckCredentialsProvider.getToken()]).then(([e2, n]) => {
      this.m_ === t && // Normally we'd have to schedule the callback on the AsyncQueue.
      // However, the following calls are safe to be called outside the
      // AsyncQueue since they don't chain asynchronous calls
      this.B_(e2, n);
    }, (t2) => {
      e(() => {
        const e2 = new FirestoreError(O.UNKNOWN, "Fetching auth token failed: " + t2.message);
        return this.L_(e2);
      });
    });
  }
  B_(e, t) {
    const n = this.N_(this.m_);
    this.stream = this.k_(e, t), this.stream.jo(() => {
      n(() => this.listener.jo());
    }), this.stream.Jo(() => {
      n(() => (this.state = 2, this.g_ = this.bi.enqueueAfterDelay(this.V_, 1e4, () => (this.b_() && (this.state = 3), Promise.resolve())), this.listener.Jo()));
    }), this.stream.Zo((e2) => {
      n(() => this.L_(e2));
    }), this.stream.onMessage((e2) => {
      n(() => 1 == ++this.p_ ? this.q_(e2) : this.onNext(e2));
    });
  }
  S_() {
    this.state = 5, this.y_.E_(async () => {
  ...
  N_(e) {
    return (t) => {
      this.bi.enqueueAndForget(() => this.m_ === e ? t() : (__PRIVATE_logDebug(Jt, "stream callback skipped by getCloseGuardedDispatcher."), Promise.resolve()));
    };
  }
};
var __PRIVATE_PersistentListenStream = class extends __PRIVATE_PersistentStream {
  ...
  K_(e) {
    const t = {};
    t.database = __PRIVATE_getEncodedDatabaseId(this.serializer), t.removeTarget = e, this.F_(t);
  }
};
var __PRIVATE_PersistentWriteStream = class extends __PRIVATE_PersistentStream {
  ...
  q_(e) {
    ...
    __PRIVATE_hardAssert(!e.writeResults || 0 === e.writeResults.length, 55816), this.listener.z_();
  }
  onNext(e) {
  ...
  H_() {
    const e = {};
    e.database = __PRIVATE_getEncodedDatabaseId(this.serializer), this.F_(e);
  }
...
var __PRIVATE_DatastoreImpl = class extends Datastore {
  ...
  Y_() {
    if (this.J_)
      throw new FirestoreError(O.FAILED_PRECONDITION, "The client has already been terminated.");
  }
  ...
  Qo(e, t, n, r) {
    return this.Y_(), Promise.all([this.authCredentials.getToken(), this.appCheckCredentials.getToken()]).then(([i, s]) => this.connection.Qo(e, __PRIVATE_toResourcePath(t, n), r, i, s)).catch((e2) => {
      throw "FirebaseError" === e2.name ? (e2.code === O.UNAUTHENTICATED && (this.authCredentials.invalidateToken(), this.appCheckCredentials.invalidateToken()), e2) : new FirestoreError(O.UNKNOWN, e2.toString());
    });
  }
  ...
  constructor(e, t, n, r, i) {
    ...
    this.ua = [], this.ca = i, this.ca.vo((e2) => {
      n.enqueueAndForget(async () => {
        __PRIVATE_canUseNetwork(this) && (__PRIVATE_logDebug(Yt, "Restarting streams for network reachability change."), await async function __PRIVATE_restartNetwork(e3) {
...
function __PRIVATE_remoteStoreListen(e, t) {
  const n = __PRIVATE_debugCast(e);
  n._a.has(t.targetId) || // Mark this as something the client is currently listening for.
  (n._a.set(t.targetId, t), __PRIVATE_shouldStartWatchStream(n) ? (
    // The listen will be sent in onWatchStreamOpen
    __PRIVATE_startWatchStream(n)
  ) : __PRIVATE_ensureWatchStream(n).b_() && __PRIVATE_sendWatchRequest(n, t));
}
function __PRIVATE_remoteStoreUnlisten(e, t) {
  const n = __PRIVATE_debugCast(e), r = __PRIVATE_ensureWatchStream(n);
  n._a.delete(t), r.b_() && __PRIVATE_sendUnwatchRequest(n, t), 0 === n._a.size && (r.b_() ? r.v_() : __PRIVATE_canUseNetwork(n) && // Revert to OnlineState.Unknown if the watch stream is not open and we
...
function __PRIVATE_sendWatchRequest(e, t) {
  if (e.ha.Ke(t.targetId), t.resumeToken.approximateByteSize() > 0 || t.snapshotVersion.compareTo(SnapshotVersion.min()) > 0) {
...
function __PRIVATE_sendUnwatchRequest(e, t) {
  e.ha.Ke(t), __PRIVATE_ensureWatchStream(e).K_(t);
}
function __PRIVATE_startWatchStream(e) {
  e.ha = new __PRIVATE_WatchChangeAggregator({
    getRemoteKeysForTarget: (t) => e.remoteSyncer.getRemoteKeysForTarget(t),
    Rt: (t) => e._a.get(t) || null,
    Pt: () => e.datastore.serializer.databaseId
  }), __PRIVATE_ensureWatchStream(e).start(), e.la.ta();
}
function __PRIVATE_shouldStartWatchStream(e) {
  return __PRIVATE_canUseNetwork(e) && !__PRIVATE_ensureWatchStream(e).w_() && e._a.size > 0;
}
...
async function __PRIVATE_onWatchStreamClose(e, t) {
  ...
  __PRIVATE_shouldStartWatchStream(e) ? (e.la.ia(t), __PRIVATE_startWatchStream(e)) : (
    ...
    try {
      await async function __PRIVATE_handleTargetError(e2, t2) {
        ...
        for (const r of t2.targetIds)
          e2._a.has(r) && (await e2.remoteSyncer.rejectListen(r, n2), e2._a.delete(r), e2.ha.removeTarget(r));
      }(e, t);
    } catch (n2) {
      __PRIVATE_logDebug(Yt, "Failed to remove targets %s: %s ", t.targetIds.join(","), n2), await __PRIVATE_disableNetworkUntilRecovery(e, n2);
    }
  else if (t instanceof __PRIVATE_DocumentWatchChange ? e.ha.Xe(t) : t instanceof __PRIVATE_ExistenceFilterChange ? e.ha.ot(t) : e.ha.nt(t), !n.isEqual(SnapshotVersion.min()))
    try {
      ...
      */
      await function __PRIVATE_raiseWatchSnapshot(e2, t3) {
        const n2 = e2.ha.It(t3);
        return n2.targetChanges.forEach((n3, r) => {
          if (n3.resumeToken.approximateByteSize() > 0) {
            const i = e2._a.get(r);
            i && e2._a.set(r, i.withResumeToken(n3.resumeToken, t3));
          }
        }), // Re-establish listens for the targets that have been invalidated by
        // existence filter mismatches.
        n2.targetMismatches.forEach((t4, n3) => {
          const r = e2._a.get(t4);
          if (!r)
            return;
          e2._a.set(t4, r.withResumeToken(ByteString.EMPTY_BYTE_STRING, r.snapshotVersion)), // Cause a hard reset by unwatching and rewatching immediately, but
      ...
      __PRIVATE_logDebug(Yt, "Failed to raise snapshot:", t2), await __PRIVATE_disableNetworkUntilRecovery(e, t2);
    }
}
async function __PRIVATE_disableNetworkUntilRecovery(e, t, n) {
  ...
  await __PRIVATE_disableNetworkInternal(e), e.la.set(
    "Offline"
    /* OnlineState.Offline */
  ...
  e.asyncQueue.enqueueRetryable(async () => {
    __PRIVATE_logDebug(Yt, "Retrying IndexedDB access"), await n(), e.aa.delete(
      1
      /* OfflineCause.IndexedDbFailed */
...
async function __PRIVATE_fillWritePipeline(e) {
  ...
  let r = t.oa.length > 0 ? t.oa[t.oa.length - 1].batchId : W;
...
function __PRIVATE_canAddToWritePipeline(e) {
  return __PRIVATE_canUseNetwork(e) && e.oa.length < 10;
}
function __PRIVATE_addToWritePipeline(e, t) {
  e.oa.push(t);
  const n = __PRIVATE_ensureWriteStream(e);
  n.b_() && n.W_ && n.G_(t.mutations);
}
function __PRIVATE_shouldStartWriteStream(e) {
  return __PRIVATE_canUseNetwork(e) && !__PRIVATE_ensureWriteStream(e).w_() && e.oa.length > 0;
}
...
async function __PRIVATE_onMutationResult(e, t, n) {
  const r = e.oa.shift(), i = MutationBatchResult.from(r, t, n);
...
async function __PRIVATE_onWriteStreamClose(e, t) {
  t && __PRIVATE_ensureWriteStream(e).W_ && // This error affects the actual write.
  await async function __PRIVATE_handleWriteError(e2, t2) {
    if (function __PRIVATE_isPermanentWriteError(e3) {
      return __PRIVATE_isPermanentError(e3) && e3 !== O.ABORTED;
    }(t2.code)) {
      const n = e2.oa.shift();
      __PRIVATE_ensureWriteStream(e2).D_(), await __PRIVATE_executeWithRecovery(e2, () => e2.remoteSyncer.rejectFailedWrite(n.batchId, t2)), // It's possible that with the completion of this mutation
...
async function __PRIVATE_remoteStoreHandleCredentialChange(e, t) {
  const n = __PRIVATE_debugCast(e);
  n.asyncQueue.verifyOperationInProgress(), __PRIVATE_logDebug(Yt, "RemoteStore received new credentials");
  ...
  n.la.set(
    "Unknown"
    /* OnlineState.Unknown */
  ), await n.remoteSyncer.handleCredentialChange(t), n.aa.delete(
    3
    /* OfflineCause.CredentialChange */
...
function __PRIVATE_ensureWatchStream(e) {
  ...
  (e.Pa = function __PRIVATE_newPersistentWatchStream(e2, t, n) {
    ...
    jo: __PRIVATE_onWatchStreamConnected.bind(null, e),
    Jo: __PRIVATE_onWatchStreamOpen.bind(null, e),
    Zo: __PRIVATE_onWatchStreamClose.bind(null, e),
    Q_: __PRIVATE_onWatchStreamChange.bind(null, e)
  }), e.ua.push(async (t) => {
    t ? (e.Pa.D_(), __PRIVATE_shouldStartWatchStream(e) ? __PRIVATE_startWatchStream(e) : e.la.set(
      "Unknown"
      /* OnlineState.Unknown */
...
function __PRIVATE_ensureWriteStream(e) {
  return e.Ta || // Create stream (but note that it is not started yet).
  (e.Ta = function __PRIVATE_newPersistentWriteStream(e2, t, n) {
    const r = __PRIVATE_debugCast(e2);
    return r.Y_(), new __PRIVATE_PersistentWriteStream(t, r.connection, r.authCredentials, r.appCheckCredentials, r.serializer, n);
  }(e.datastore, e.asyncQueue, {
    jo: () => Promise.resolve(),
    Jo: __PRIVATE_onWriteStreamOpen.bind(null, e),
    Zo: __PRIVATE_onWriteStreamClose.bind(null, e),
    z_: __PRIVATE_onWriteHandshakeComplete.bind(null, e),
    j_: __PRIVATE_onMutationResult.bind(null, e)
  }), e.ua.push(async (t) => {
    t ? (e.Ta.D_(), // This will start the write stream if necessary.
    await __PRIVATE_fillWritePipeline(e)) : (await e.Ta.stop(), e.oa.length > 0 && (__PRIVATE_logDebug(Yt, `Stopping write stream with ${e.oa.length} pending writes`), e.oa = []));
  })), e.Ta;
}
var DelayedOperation = class _DelayedOperation {
   ...
   */
  cancel(e) {
    null !== this.timerHandle && (this.clearTimeout(), this.deferred.reject(new FirestoreError(O.CANCELLED, "Operation cancelled" + (e ? ": " + e : ""))));
  }
  handleDelayElapsed() {
...
function __PRIVATE_wrapInUserErrorIfRecoverable(e, t) {
  if (__PRIVATE_logError("AsyncQueue", `${t}: ${e}`), __PRIVATE_isIndexedDbTransactionError(e))
    return new FirestoreError(O.UNAVAILABLE, `${t}: ${e}`);
  throw e;
}
...
var __PRIVATE_DocumentChangeSet = class {
  constructor() {
    this.Ia = new SortedMap(DocumentKey.comparator);
  }
  track(e) {
    const t = e.doc.key, n = this.Ia.get(t);
    n ? (
      // Merge the new change with the existing change.
      0 !== e.type && 3 === n.type ? this.Ia = this.Ia.insert(t, e) : 3 === e.type && 1 !== n.type ? this.Ia = this.Ia.insert(t, {
        type: n.type,
        doc: e.doc
      }) : 2 === e.type && 2 === n.type ? this.Ia = this.Ia.insert(t, {
        type: 2,
        doc: e.doc
      }) : 2 === e.type && 0 === n.type ? this.Ia = this.Ia.insert(t, {
        type: 0,
        doc: e.doc
      }) : 1 === e.type && 0 === n.type ? this.Ia = this.Ia.remove(t) : 1 === e.type && 2 === n.type ? this.Ia = this.Ia.insert(t, {
        type: 1,
        doc: n.doc
      }) : 0 === e.type && 1 === n.type ? this.Ia = this.Ia.insert(t, {
        type: 2,
        doc: e.doc
      }) : (
  ...
  da() {
    const e = [];
    return this.Ia.inorderTraversal((t, n) => {
      e.push(n);
    }), e;
  }
};
  ...
  terminate() {
    !function __PRIVATE_errorAllTargets(e, t) {
      ...
      n.queries = __PRIVATE_newQueriesObjectMap(), r.forEach((e2, n2) => {
        for (const e3 of n2.Ra)
          e3.onError(t);
      });
    }(this, new FirestoreError(O.ABORTED, "Firestore shutting down"));
  }
};
...
async function __PRIVATE_eventManagerListen(e, t) {
  ...
  s ? !s.Va() && t.ma() && // Query has been listening to local cache, and tries to add a new listener sourced from watch.
  (r = 2) : (s = new __PRIVATE_QueryListenersInfo(), r = t.ma() ? 0 : 1);
  try {
    switch (r) {
      case 0:
        s.Aa = await n.onListen(
          i,
          /** enableRemoteListen= */
          true
        );
  ...
  if (n.queries.set(i, s), s.Ra.push(t), // Run global snapshot listeners if a consistent snapshot has been emitted.
  t.ga(n.onlineState), s.Aa) {
    t.pa(s.Aa) && __PRIVATE_raiseSnapshotsInSyncEvent(n);
  }
}
async function __PRIVATE_eventManagerUnlisten(e, t) {
  ...
  if (s) {
    const e2 = s.Ra.indexOf(t);
    e2 >= 0 && (s.Ra.splice(e2, 1), 0 === s.Ra.length ? i = t.ma() ? 0 : 1 : !s.Va() && t.ma() && // The removed listener is the last one that sourced from watch.
    (i = 2));
  }
  switch (i) {
    case 0:
...
var Zt;
var Xt;
(Xt = Zt || (Zt = {})).ya = "default", /** Listen to changes in cache only */
Xt.Cache = "cache";
var __PRIVATE_QueryListener = class {
  constructor(e, t, n) {
    this.query = e, this.wa = t, /**
     * Initial snapshots (e.g. from cache) may not be propagated to the wrapped
     * observer. This flag is set to true once we've actually raised an event.
     */
    this.ba = false, this.Sa = null, this.onlineState = "Unknown", this.options = n || {};
  }
  /**
  ...
  pa(e) {
    ...
    let t = false;
    return this.ba ? this.Da(e) && (this.wa.next(e), t = true) : this.va(e, this.onlineState) && (this.Ca(e), t = true), this.Sa = e, t;
  }
  onError(e) {
    this.wa.error(e);
  }
  ...
  ga(e) {
    this.onlineState = e;
    let t = false;
    return this.Sa && !this.ba && this.va(this.Sa, e) && (this.Ca(this.Sa), t = true), t;
  }
  va(e, t) {
    if (!e.fromCache)
      return true;
    if (!this.ma())
      return true;
    const n = "Offline" !== t;
    return (!this.options.Fa || !n) && (!e.docs.isEmpty() || e.hasCachedResults || "Offline" === t);
  }
  Da(e) {
  ...
  Ca(e) {
    e = ViewSnapshot.fromInitialDocuments(e.query, e.docs, e.mutatedKeys, e.fromCache, e.hasCachedResults), this.ba = true, this.wa.next(e);
  }
  ma() {
    return this.options.source !== Zt.Cache;
  }
};
var __PRIVATE_SizedBundleElement = class {
  constructor(e, t) {
    this.Ma = e, this.byteLength = t;
  }
  xa() {
...
var __PRIVATE_BundleConverterImpl = class {
   ...
   */
  Ls(e) {
    return e.metadata.exists ? __PRIVATE_fromDocument(this.serializer, e.document, false) : MutableDocument.newNoDocument(this.Bs(e.metadata.name), this.ks(e.metadata.readTime));
  }
  ks(e) {
...
var __PRIVATE_BundleLoader = class {
  ...
  Na(e) {
    ...
    if (e.Ma.namedQuery)
      ...
      const n = ResourcePath.fromString(e.Ma.documentMetadata.name);
      this.collectionGroups.add(n.get(n.length - 2));
    } else
      e.Ma.document && (this.documents[this.documents.length - 1].document = e.Ma.document, ++t);
   ...
   */
  async complete() {
    const e = await __PRIVATE_localStoreApplyBundledDocuments(this.localStore, new __PRIVATE_BundleConverterImpl(this.serializer), this.documents, this.Oa.id), t = this.Ba(this.documents);
...
var __PRIVATE_View = class {
  constructor(e, t) {
    this.query = e, this.qa = t, this.Qa = null, this.hasCachedResults = false, /**
    ...
    this.$a = __PRIVATE_documentKeySet(), /** Document Keys that have local changes */
    this.mutatedKeys = __PRIVATE_documentKeySet(), this.Ua = __PRIVATE_newQueryComparator(e), this.Ka = new DocumentSet(this.Ua);
  }
  /**
   ...
   */
  Ga(e, t) {
    const n = t ? t.za : new __PRIVATE_DocumentChangeSet(), r = t ? t.Ka : this.Ka;
    ...
    if (e.inorderTraversal((e2, t2) => {
      ...
      if (u && c) {
        u.data.isEqual(c.data) ? l !== h && (n.track({
          type: 3,
          doc: c
        }), P = true) : this.ja(u, c) || (n.track({
          type: 2,
          doc: c
        }), P = true, (_ && this.Ua(c, _) > 0 || a && this.Ua(c, a) < 0) && // This doc moved from inside the limit to outside the limit.
  ...
  applyChanges(e, t, n, r) {
    const i = this.Ka;
    this.Ka = e.Ka, this.mutatedKeys = e.mutatedKeys;
    const s = e.za.da();
    ...
    }(e2.type, t2.type) || this.Ua(e2.doc, t2.doc)), this.Ha(n), r = null != r && r;
    const o = t && !r ? this.Ja() : [], _ = 0 === this.$a.size && this.current && !r ? 1 : 0, a = _ !== this.Qa;
    if (this.Qa = _, 0 !== s.length || a) {
      return {
  ...
  ga(e) {
    return this.current && "Offline" === e ? (
      ...
      (this.current = false, this.applyChanges(
        {
          Ka: this.Ka,
          za: new __PRIVATE_DocumentChangeSet(),
   ...
   */
  Za(e) {
    return !this.qa.has(e) && // The local store doesn't think it's a result, so it shouldn't be in limbo.
    (!!this.Ka.has(e) && !this.Ka.get(e).hasLocalMutations);
  }
  /**
   ...
   */
  Ha(e) {
    e && (e.addedDocuments.forEach((e2) => this.qa = this.qa.add(e2)), e.modifiedDocuments.forEach((e2) => {
    }), e.removedDocuments.forEach((e2) => this.qa = this.qa.delete(e2)), this.current = e.current);
  }
  Ja() {
    if (!this.current)
      return [];
    const e = this.$a;
    this.$a = __PRIVATE_documentKeySet(), this.Ka.forEach((e2) => {
      this.Za(e2.key) && (this.$a = this.$a.add(e2.key));
    });
    const t = [];
    return e.forEach((e2) => {
      this.$a.has(e2) || t.push(new __PRIVATE_RemovedLimboDocument(e2));
    }), this.$a.forEach((n) => {
  ...
  // PORTING NOTE: Multi-tab only.
  Xa(e) {
    this.qa = e.Ns, this.$a = __PRIVATE_documentKeySet();
  ...
  // PORTING NOTE: Multi-tab only.
  eu() {
    return ViewSnapshot.fromInitialDocuments(this.query, this.Ka, this.mutatedKeys, 0 === this.Qa, this.hasCachedResults);
  }
};
var en = "SyncEngine";
...
async function __PRIVATE_syncEngineListen(e, t, n = true) {
  ...
  return s ? (
    ...
    (r.sharedClientState.addLocalQueryTarget(s.targetId), i = s.view.eu())
...
async function __PRIVATE_initializeViewAndComputeSnapshot(e, t, n, r, i) {
  e.Pu = (t2, n2, r2) => async function __PRIVATE_applyDocChanges(e2, t3, n3, r3) {
    ...
    i2.ys && // The query has a limit and some docs were removed, so we need
    ...
    (i2 = await __PRIVATE_localStoreExecuteQuery(
      ...
      /* usePreviousResults= */
      false
    ).then(({ documents: e3 }) => t3.view.Ga(e3, i2)));
  ...
  const s = await __PRIVATE_localStoreExecuteQuery(
    ...
    /* usePreviousResults= */
    true
  ), o = new __PRIVATE_View(t, s.Ns), _ = o.Ga(s.documents), a = TargetChange.createSynthesizedTargetChangeForCurrentChange(n, r && "Offline" !== e.onlineState, i), u = o.applyChanges(
    _,
    /* limboResolutionEnabled= */
    e.isPrimaryClient,
    a
  ...
  return e.ru.set(t, c), e.iu.has(n) ? e.iu.get(n).push(t) : e.iu.set(n, [t]), u.snapshot;
}
async function __PRIVATE_syncEngineUnlisten(e, t, n) {
  const r = __PRIVATE_debugCast(e), i = r.ru.get(t), s = r.iu.get(i.targetId);
  if (s.length > 1)
    return r.iu.set(i.targetId, s.filter((e2) => !__PRIVATE_queryEquals(e2, t))), void r.ru.delete(t);
...
async function __PRIVATE_triggerRemoteStoreUnlisten(e, t) {
  const n = __PRIVATE_debugCast(e), r = n.ru.get(t), i = n.iu.get(r.targetId);
...
async function __PRIVATE_syncEngineWrite(e, t, n) {
  ...
  try {
    ...
    r.sharedClientState.addPendingMutation(e2.batchId), function __PRIVATE_addMutationCallback(e3, t2, n2) {
      let r2 = e3.uu[e3.currentUser.toKey()];
      r2 || (r2 = new SortedMap(__PRIVATE_primitiveComparator));
      r2 = r2.insert(t2, n2), e3.uu[e3.currentUser.toKey()] = r2;
...
async function __PRIVATE_syncEngineApplyRemoteEvent(e, t) {
  ...
  try {
    ...
    t.targetChanges.forEach((e3, t2) => {
      ...
      (__PRIVATE_hardAssert(e3.addedDocuments.size + e3.modifiedDocuments.size + e3.removedDocuments.size <= 1, 22616), e3.addedDocuments.size > 0 ? r.tu = true : e3.modifiedDocuments.size > 0 ? __PRIVATE_hardAssert(r.tu, 14607) : e3.removedDocuments.size > 0 && (__PRIVATE_hardAssert(r.tu, 42227), r.tu = false));
...
async function __PRIVATE_syncEngineRejectListen(e, t, n) {
  ...
  if (s) {
    ...
    r.ou = r.ou.remove(s), r._u.delete(t), __PRIVATE_pumpEnqueuedLimboResolutions(r);
  } else
...
async function __PRIVATE_syncEngineRegisterPendingWritesCallback(e, t) {
  const n = __PRIVATE_debugCast(e);
  __PRIVATE_canUseNetwork(n.remoteStore) || __PRIVATE_logDebug(en, "The network is disabled. The task returned by 'awaitPendingWrites()' will not complete until the network is enabled.");
  try {
...
function __PRIVATE_triggerPendingWritesCallbacks(e, t) {
  (e.cu.get(t) || []).forEach((e2) => {
    e2.resolve();
  }), e.cu.delete(t);
}
function __PRIVATE_processUserCallback(e, t, n) {
  const r = __PRIVATE_debugCast(e);
  let i = r.uu[r.currentUser.toKey()];
  if (i) {
    const e2 = i.get(t);
    e2 && (n ? e2.reject(n) : e2.resolve(), i = i.remove(t)), r.uu[r.currentUser.toKey()] = i;
  }
}
function __PRIVATE_removeAndCleanupTarget(e, t, n = null) {
  e.sharedClientState.removeLocalQueryTarget(t);
  for (const r of e.iu.get(t))
    e.ru.delete(r), n && e.nu.Tu(r, n);
  if (e.iu.delete(t), e.isPrimaryClient) {
    e.au.Ur(t).forEach((t2) => {
...
function __PRIVATE_removeLimboTarget(e, t) {
  ...
  const n = e.ou.get(t);
  null !== n && (__PRIVATE_remoteStoreUnlisten(e.remoteStore, n), e.ou = e.ou.remove(t), e._u.delete(n), __PRIVATE_pumpEnqueuedLimboResolutions(e));
}
function __PRIVATE_updateTrackedLimbos(e, t, n) {
  for (const r of n)
    if (r instanceof __PRIVATE_AddedLimboDocument)
      ...
      __PRIVATE_logDebug(en, "Document no longer in limbo: " + r.key), e.au.removeReference(r.key, t);
...
function __PRIVATE_trackLimboChange(e, t) {
  ...
  e.ou.get(n) || e.su.has(r) || (__PRIVATE_logDebug(en, "New document in limbo: " + n), e.su.add(r), __PRIVATE_pumpEnqueuedLimboResolutions(e));
}
function __PRIVATE_pumpEnqueuedLimboResolutions(e) {
  for (; e.su.size > 0 && e.ou.size < e.maxConcurrentLimboResolutions; ) {
    const t = e.su.values().next().value;
    e.su.delete(t);
    const n = new DocumentKey(ResourcePath.fromString(t)), r = e.lu.next();
    e._u.set(r, new LimboResolution(n)), e.ou = e.ou.insert(n, r), __PRIVATE_remoteStoreListen(e.remoteStore, new TargetData(__PRIVATE_queryToTarget(__PRIVATE_newQueryForPath(n.path)), r, "TargetPurposeLimboResolution", __PRIVATE_ListenSequence.le));
  }
}
  ...
  r.ru.isEmpty() || (r.ru.forEach((e2, _) => {
    ...
    try {
      ...
      if (!__PRIVATE_isIndexedDbTransactionError(e3))
        throw e3;
      __PRIVATE_logDebug(kt, "Failed to update sequence numbers: " + e3);
    }
    for (const e3 of t2) {
...
async function __PRIVATE_syncEngineHandleCredentialChange(e, t) {
  ...
  if (!n.currentUser.isEqual(t)) {
    __PRIVATE_logDebug(en, "User change. New user:", t.toKey());
    ...
    function __PRIVATE_rejectOutstandingPendingWritesCallbacks(e3, t2) {
      e3.cu.forEach((e4) => {
        e4.forEach((e5) => {
          e5.reject(new FirestoreError(O.CANCELLED, t2));
        });
      }), e3.cu.clear();
...
function __PRIVATE_syncEngineGetRemoteKeysForTarget(e, t) {
  const n = __PRIVATE_debugCast(e), r = n._u.get(t);
  if (r && r.tu)
...
async function __PRIVATE_syncEngineApplyBatchState(e, t, n, r) {
  const i = __PRIVATE_debugCast(e), s = await function __PRIVATE_localStoreLookupMutationDocuments(e2, t2) {
    ...
    return n2.persistence.runTransaction("Lookup mutation documents", "readonly", (e3) => r2.Hn(e3, t2).next((t3) => t3 ? n2.localDocuments.getDocuments(e3, t3) : PersistencePromise.resolve(null)));
  }(i.localStore, t);
  null !== s ? ("pending" === n ? (
    ...
    // as applicable.
    __PRIVATE_logDebug(en, "Cannot apply mutation batch with id: " + t)
  );
}
async function __PRIVATE_syncEngineApplyPrimaryState(e, t) {
  const n = __PRIVATE_debugCast(e);
  if (__PRIVATE_ensureWatchCallbacks(n), __PRIVATE_syncEngineEnsureWriteCallbacks(n), true === t && true !== n.hu) {
    ...
    n.hu = true, await __PRIVATE_remoteStoreApplyPrimaryState(n.remoteStore, true);
    for (const e3 of t2)
      __PRIVATE_remoteStoreListen(n.remoteStore, e3);
  } else if (false === t && false !== n.hu) {
    const e2 = [];
    ...
    function __PRIVATE_resetLimboDocuments(e3) {
      ...
      }), t3.au.Kr(), t3._u = /* @__PURE__ */ new Map(), t3.ou = new SortedMap(DocumentKey.comparator);
    }(n), n.hu = false, await __PRIVATE_remoteStoreApplyPrimaryState(n.remoteStore, false);
  }
}
async function __PRIVATE_synchronizeQueryViewsAndRaiseSnapshots(e, t, n) {
  ...
  for (const e2 of t) {
    ...
    if (n2 && 0 !== n2.length) {
      ...
      for (const e3 of n2) {
        const t3 = r.ru.get(e3), n3 = await __PRIVATE_synchronizeViewAndComputeSnapshot(r, t3);
...
function __PRIVATE_syncEngineGetActiveClients(e) {
  return function __PRIVATE_localStoreGetActiveClients(e2) {
    return __PRIVATE_debugCast(__PRIVATE_debugCast(e2).persistence).us();
  }(__PRIVATE_debugCast(e).localStore);
}
async function __PRIVATE_syncEngineApplyTargetState(e, t, n, r) {
  const i = __PRIVATE_debugCast(e);
  if (i.hu)
    return void __PRIVATE_logDebug(en, "Ignoring unexpected query state notification.");
  const s = i.iu.get(t);
  if (s && s.length > 0)
    switch (n) {
      case "current":
  ...
  if (r.hu) {
    for (const e2 of t) {
      if (r.iu.has(e2) && r.sharedClientState.isActiveQueryTarget(e2)) {
        __PRIVATE_logDebug(en, "Adding an already active target " + e2);
        continue;
      }
...
function __PRIVATE_ensureWatchCallbacks(e) {
  const t = __PRIVATE_debugCast(e);
  return t.remoteStore.remoteSyncer.applyRemoteEvent = __PRIVATE_syncEngineApplyRemoteEvent.bind(null, t), t.remoteStore.remoteSyncer.getRemoteKeysForTarget = __PRIVATE_syncEngineGetRemoteKeysForTarget.bind(null, t), t.remoteStore.remoteSyncer.rejectListen = __PRIVATE_syncEngineRejectListen.bind(null, t), t.nu.Q_ = __PRIVATE_eventManagerOnWatchChange.bind(null, t.eventManager), t.nu.Tu = __PRIVATE_eventManagerOnWatchError.bind(null, t.eventManager), t;
}
  ...
  (async function __PRIVATE_loadBundleImpl(e2, t2, n2) {
    try {
      ...
      if (await function __PRIVATE_localStoreHasNewerBundle(e3, t3) {
        ...
        return n3.persistence.runTransaction("hasNewerBundle", "readonly", (e4) => n3.ci.getBundleMetadata(e4, t3.id)).then((e4) => !!e4 && e4.createTime.compareTo(r3) >= 0);
      }(e2.localStore, r2))
      ...
      for (; s; ) {
        ...
        e3 && n2._updateProgress(e3), s = await t2.du();
      }
      ...
      await function __PRIVATE_localStoreSaveBundle(e3, t3) {
        ...
        return n3.persistence.runTransaction("Save bundle", "readwrite", (e4) => n3.ci.saveBundleMetadata(e4, t3));
      }(e2.localStore, r2), n2._completeWith(o.progress), Promise.resolve(o.La);
    } catch (e3) {
      return __PRIVATE_logWarn(en, `Loading bundle failed with ${e3}`), n2._failWith(e3), Promise.resolve(/* @__PURE__ */ new Set());
    }
  })(r, t, n).then((e2) => {
...
var __PRIVATE_IndexedDbOfflineComponentProvider = class extends __PRIVATE_MemoryOfflineComponentProvider {
  constructor(e, t, n) {
    super(), this.gu = e, this.cacheSizeBytes = t, this.forceOwnership = n, this.kind = "persistent", this.synchronizeTabs = false;
  }
  async initialize(e) {
    await super.initialize(e), await this.gu.initialize(this, e), // Enqueue writes from a previous session
    await __PRIVATE_syncEngineEnsureWriteCallbacks(this.gu.syncEngine), await __PRIVATE_fillWritePipeline(this.gu.remoteStore), // NOTE: This will immediately call the listener, so we make sure to
    // set it after localStore / remoteStore are started.
    await this.persistence.Ki(() => (this.gcScheduler && !this.gcScheduler.started && this.gcScheduler.start(), this.indexBackfillerScheduler && !this.indexBackfillerScheduler.started && this.indexBackfillerScheduler.start(), Promise.resolve()));
  }
  Vu(e) {
    return __PRIVATE_newLocalStore(this.persistence, new __PRIVATE_QueryEngine(), e.initialUser, this.serializer);
  }
...
var __PRIVATE_MultiTabOfflineComponentProvider = class extends __PRIVATE_IndexedDbOfflineComponentProvider {
  constructor(e, t) {
    super(
      ...
      /* forceOwnership= */
      false
    ), this.gu = e, this.cacheSizeBytes = t, this.synchronizeTabs = true;
  }
  async initialize(e) {
    await super.initialize(e);
    const t = this.gu.syncEngine;
    this.sharedClientState instanceof __PRIVATE_WebStorageSharedClientState && (this.sharedClientState.syncEngine = {
      yo: __PRIVATE_syncEngineApplyBatchState.bind(null, t),
      wo: __PRIVATE_syncEngineApplyTargetState.bind(null, t),
      bo: __PRIVATE_syncEngineApplyActiveTargetsChange.bind(null, t),
      us: __PRIVATE_syncEngineGetActiveClients.bind(null, t),
      po: __PRIVATE_syncEngineSynchronizeWithChangedDocuments.bind(null, t)
    }, await this.sharedClientState.start()), // NOTE: This will immediately call the listener, so we make sure to
    // set it after localStore / remoteStore are started.
    await this.persistence.Ki(async (e2) => {
      await __PRIVATE_syncEngineApplyPrimaryState(this.gu.syncEngine, e2), this.gcScheduler && (e2 && !this.gcScheduler.started ? this.gcScheduler.start() : e2 || this.gcScheduler.stop()), this.indexBackfillerScheduler && (e2 && !this.indexBackfillerScheduler.started ? this.indexBackfillerScheduler.start() : e2 || this.indexBackfillerScheduler.stop());
    });
  }
  Au(e) {
    const t = __PRIVATE_getWindow();
    if (!__PRIVATE_WebStorageSharedClientState.C(t))
      throw new FirestoreError(O.UNIMPLEMENTED, "IndexedDB persistence is only available on platforms that support LocalStorage.");
...
var OnlineComponentProvider = class {
  ...
  async terminate() {
    ...
    await async function __PRIVATE_remoteStoreShutdown(e2) {
      ...
      __PRIVATE_logDebug(Yt, "RemoteStore shutting down."), t2.aa.add(
        5
...
var __PRIVATE_AsyncObserver = class {
  ...
  error(e) {
    this.muted || (this.observer.error ? this.pu(this.observer.error, e) : __PRIVATE_logError("Uncaught Error in snapshot listener:", e.toString()));
  }
  yu() {
    this.muted = true;
  }
...
var __PRIVATE_BundleReaderImpl = class {
  constructor(e, t) {
    this.wu = e, this.serializer = t, /** Cached bundle metadata. */
    ...
    this.Su().then((e2) => {
      e2 && e2.xa() ? this.metadata.resolve(e2.Ma.metadata) : this.metadata.reject(new Error(`The first element of the bundle is not a metadata, it is
             ${JSON.stringify(null == e2 ? void 0 : e2.Ma)}`));
  ...
  async du() {
    return await this.getMetadata(), this.Su();
  }
  /**
   ...
   */
  async Su() {
    const e = await this.Du();
    if (null === e)
      return null;
    const t = this.bu.decode(e), n = Number(t);
    isNaN(n) && this.vu(`length string (${t}) is not valid number`);
    const r = await this.Cu(n);
  ...
  vu(e) {
    throw this.wu.cancel(), new Error(`Invalid bundle format: ${e}`);
  }
  /**
...
var Transaction$2 = class {
  ...
  async lookup(e) {
    if (this.ensureCommitNotCalled(), this.mutations.length > 0)
      throw this.lastTransactionError = new FirestoreError(O.INVALID_ARGUMENT, "Firestore transactions require all reads to be executed before all writes."), this.lastTransactionError;
    const t = await async function __PRIVATE_invokeBatchGetDocumentsRpc(e2, t2) {
      ...
      }, i = await n.Wo("BatchGetDocuments", n.serializer.databaseId, ResourcePath.emptyPath(), r, t2.length), s = /* @__PURE__ */ new Map();
      i.forEach((e3) => {
  ...
  async commit() {
    ...
    e.forEach((e2, t) => {
      ...
      await n.Qo("Commit", n.serializer.databaseId, ResourcePath.emptyPath(), r);
  ...
  recordVersion(e) {
    ...
    if (n) {
      if (!t.isEqual(n))
        throw new FirestoreError(O.ABORTED, "Document version changed between two reads.");
    } else
  ...
  preconditionForUpdate(e) {
    ...
    if (!this.writtenDocs.has(e.toString()) && t) {
      if (t.isEqual(SnapshotVersion.min()))
        throw new FirestoreError(O.INVALID_ARGUMENT, "Can't update a document that doesn't exist.");
...
var __PRIVATE_TransactionRunner = class {
  constructor(e, t, n, r, i) {
    this.asyncQueue = e, this.datastore = t, this.options = n, this.updateFunction = r, this.deferred = i, this.Ou = n.maxAttempts, this.y_ = new __PRIVATE_ExponentialBackoff(
      this.asyncQueue,
      "transaction_retry"
  ...
  Bu() {
    this.y_.E_(async () => {
      const e = new Transaction$2(this.datastore), t = this.Lu(e);
      t && t.then((t2) => {
  ...
  ku(e) {
    this.Ou > 0 && this.qu(e) ? (this.Ou -= 1, this.asyncQueue.enqueueAndForget(() => (this.Bu(), Promise.resolve()))) : this.deferred.reject(e);
  }
  qu(e) {
...
var FirestoreClient = class {
  constructor(e, t, n, r, i) {
    this.authCredentials = e, this.appCheckCredentials = t, this.asyncQueue = n, this.databaseInfo = r, this.user = User.UNAUTHENTICATED, this.clientId = __PRIVATE_AutoId.newId(), this.authCredentialListener = () => Promise.resolve(), this.appCheckCredentialListener = () => Promise.resolve(), this._uninitializedComponentsProvider = i, this.authCredentials.start(n, async (e2) => {
      __PRIVATE_logDebug(tn, "Received user=", e2.uid), await this.authCredentialListener(e2), this.user = e2;
    }), this.appCheckCredentials.start(n, (e2) => (__PRIVATE_logDebug(tn, "Received new app check token=", e2), this.appCheckCredentialListener(e2, this.user)));
  }
  get configuration() {
    return {
...
async function __PRIVATE_setOfflineComponentProvider(e, t) {
  e.asyncQueue.verifyOperationInProgress(), __PRIVATE_logDebug(tn, "Initializing OfflineComponentProvider");
...
async function __PRIVATE_setOnlineComponentProvider(e, t) {
  ...
  __PRIVATE_logDebug(tn, "Initializing OnlineComponentProvider"), await t.initialize(n, e.configuration), // The CredentialChangeListener of the online component provider takes
    ...
    if (e._uninitializedComponentsProvider) {
      ...
      try {
        ...
        if (!function __PRIVATE_canFallbackFromIndexedDbError(e2) {
          return "FirebaseError" === e2.name ? e2.code === O.FAILED_PRECONDITION || e2.code === O.UNIMPLEMENTED : !("undefined" != typeof DOMException && e2 instanceof DOMException) || // When the browser is out of quota we could get either quota exceeded
      ...
      __PRIVATE_logDebug(tn, "Using default OfflineComponentProvider"), await __PRIVATE_setOfflineComponentProvider(e, new __PRIVATE_LruGcMemoryOfflineComponentProvider(void 0));
...
function __PRIVATE_firestoreClientGetDocumentFromLocalCache(e, t) {
  ...
  return e.asyncQueue.enqueueAndForget(async () => async function __PRIVATE_readDocumentFromCache(e2, t2, n2) {
    try {
      ...
      r.isFoundDocument() ? n2.resolve(r) : r.isNoDocument() ? n2.resolve(null) : n2.reject(new FirestoreError(O.UNAVAILABLE, "Failed to get document from cache. (However, this document may exist on the server. Run again without setting 'source' in the GetOptions to attempt to retrieve the document from the server.)"));
    } catch (e3) {
...
function __PRIVATE_firestoreClientGetDocumentViaSnapshotListener(e, t, n = {}) {
  ...
  return e.asyncQueue.enqueueAndForget(async () => function __PRIVATE_readDocumentViaSnapshotListener(e2, t2, n2, r2, i) {
    const s = new __PRIVATE_AsyncObserver({
      next: (_) => {
        s.yu(), t2.enqueueAndForget(() => __PRIVATE_eventManagerUnlisten(e2, o));
        ...
        !a && _.fromCache ? (
          ...
          i.reject(new FirestoreError(O.UNAVAILABLE, "Failed to get document because the client is offline."))
        ) : a && _.fromCache && r2 && "server" === r2.source ? i.reject(new FirestoreError(O.UNAVAILABLE, 'Failed to get document from server. (However, this document does exist in the local cache. Run again without setting source to "server" to retrieve the cached document.)')) : i.resolve(_);
      },
  ...
  return e.asyncQueue.enqueueAndForget(async () => async function __PRIVATE_executeQueryFromCache(e2, t2, n2) {
    try {
      const r = await __PRIVATE_localStoreExecuteQuery(
        ...
        /* usePreviousResults= */
        true
      ), i = new __PRIVATE_View(t2, r.Ns), s = i.Ga(r.documents), o = i.applyChanges(
        s,
        /* limboResolutionEnabled= */
        false
      );
...
function __PRIVATE_firestoreClientGetDocumentsViaSnapshotListener(e, t, n = {}) {
  ...
  return e.asyncQueue.enqueueAndForget(async () => function __PRIVATE_executeQueryViaSnapshotListener(e2, t2, n2, r2, i) {
    const s = new __PRIVATE_AsyncObserver({
      next: (n3) => {
        s.yu(), t2.enqueueAndForget(() => __PRIVATE_eventManagerUnlisten(e2, o)), n3.fromCache && "server" === r2.source ? i.reject(new FirestoreError(O.UNAVAILABLE, 'Failed to get documents from server. (However, these documents may exist in the local cache. Run again without setting source to "server" to retrieve the cached documents.)')) : i.resolve(n3);
      },
      error: (e3) => i.reject(e3)
  ...
  return e.asyncQueue.enqueueAndForget(async () => {
    try {
      ...
      r.resolve(async function __PRIVATE_invokeRunAggregationQueryRpc(e2, t2, n2) {
        ...
        i2.connection.Bo || delete s.parent;
        const a = (await i2.Wo(
          "RunAggregationQuery",
...
function __PRIVATE_firestoreClientAddSnapshotsInSyncListener(e, t) {
  ...
  return e.asyncQueue.enqueueAndForget(async () => function __PRIVATE_addSnapshotsInSyncListener(e2, t2) {
    __PRIVATE_debugCast(e2).fa.add(t2), // Immediately fire an initial event, indicating all existing listeners
    // are in-sync.
    t2.next();
  }(await __PRIVATE_getEventManager(e), n)), () => {
    n.yu(), e.asyncQueue.enqueueAndForget(async () => function __PRIVATE_removeSnapshotsInSyncListener(e2, t2) {
      __PRIVATE_debugCast(e2).fa.delete(t2);
    }(await __PRIVATE_getEventManager(e), n));
  };
}
...
function __PRIVATE_firestoreClientGetNamedQuery(e, t) {
  return e.asyncQueue.enqueue(async () => function __PRIVATE_localStoreGetNamedQuery(e2, t2) {
    ...
    return n.persistence.runTransaction("Get named query", "readonly", (e3) => n.ci.getNamedQuery(e3, t2));
...
function __PRIVATE_firestoreClientSetPersistentCacheIndexAutoCreationEnabled(e, t) {
  return e.asyncQueue.enqueue(async () => function __PRIVATE_localStoreSetIndexAutoCreationEnabled(e2, t2) {
    __PRIVATE_debugCast(e2).bs.Is = t2;
  }(await __PRIVATE_getLocalStore(e), t));
}
...
function __PRIVATE_validateNonEmptyArgument(e, t, n) {
  if (!n)
    throw new FirestoreError(O.INVALID_ARGUMENT, `Function ${e}() cannot be called with an empty ${t}.`);
}
function __PRIVATE_validateIsNotUsedTogether(e, t, n, r) {
  if (true === t && true === r)
    throw new FirestoreError(O.INVALID_ARGUMENT, `${e} and ${n} cannot be used together.`);
}
function __PRIVATE_validateDocumentPath(e) {
  if (!DocumentKey.isDocumentKey(e))
    throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid document reference. Document references must have an even number of segments, but ${e} has ${e.length}.`);
}
function __PRIVATE_validateCollectionPath(e) {
  if (DocumentKey.isDocumentKey(e))
    throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid collection reference. Collection references must have an odd number of segments, but ${e} has ${e.length}.`);
}
...
function __PRIVATE_cast(e, t) {
  ...
  (e = e._delegate), !(e instanceof t)) {
    if (t.name === e.constructor.name)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?");
    {
      const n = __PRIVATE_valueDescription(e);
      throw new FirestoreError(O.INVALID_ARGUMENT, `Expected type '${t.name}', but it was: ${n}`);
    }
  }
  return e;
}
function __PRIVATE_validatePositiveNumber(e, t) {
  if (t <= 0)
    throw new FirestoreError(O.INVALID_ARGUMENT, `Function ${e}() requires a positive number, but it was: ${t}.`);
}
var rn = "firestore.googleapis.com";
var sn = true;
var FirestoreSettingsImpl = class {
  constructor(e) {
    ...
    if (void 0 === e.host) {
      if (void 0 !== e.ssl)
        throw new FirestoreError(O.INVALID_ARGUMENT, "Can't provide ssl option if host option is not set");
      this.host = rn, this.ssl = sn;
    } else
      this.host = e.host, this.ssl = null !== (t = e.ssl) && void 0 !== t ? t : sn;
    if (this.isUsingEmulator = void 0 !== e.emulatorOptions, this.credentials = e.credentials, this.ignoreUndefinedProperties = !!e.ignoreUndefinedProperties, this.localCache = e.localCache, void 0 === e.cacheSizeBytes)
      this.cacheSizeBytes = Ct;
    else {
      if (-1 !== e.cacheSizeBytes && e.cacheSizeBytes < Mt)
        throw new FirestoreError(O.INVALID_ARGUMENT, "cacheSizeBytes must be at least 1048576");
    ...
    __PRIVATE_validateIsNotUsedTogether("experimentalForceLongPolling", e.experimentalForceLongPolling, "experimentalAutoDetectLongPolling", e.experimentalAutoDetectLongPolling), this.experimentalForceLongPolling = !!e.experimentalForceLongPolling, this.experimentalForceLongPolling ? this.experimentalAutoDetectLongPolling = false : void 0 === e.experimentalAutoDetectLongPolling ? this.experimentalAutoDetectLongPolling = true : (
      ...
      if (void 0 !== e2.timeoutSeconds) {
        if (isNaN(e2.timeoutSeconds))
          throw new FirestoreError(O.INVALID_ARGUMENT, `invalid long polling timeout: ${e2.timeoutSeconds} (must not be NaN)`);
        if (e2.timeoutSeconds < 5)
          throw new FirestoreError(O.INVALID_ARGUMENT, `invalid long polling timeout: ${e2.timeoutSeconds} (minimum allowed value is 5)`);
        if (e2.timeoutSeconds > 30)
          throw new FirestoreError(O.INVALID_ARGUMENT, `invalid long polling timeout: ${e2.timeoutSeconds} (maximum allowed value is 30)`);
      }
...
var Firestore$1 = class {
  ...
  get app() {
    if (!this._app)
      throw new FirestoreError(O.FAILED_PRECONDITION, "Firestore was not initialized using the Firebase SDK. 'app' is not available");
    return this._app;
  }
  get _initialized() {
  ...
  _setSettings(e) {
    if (this._settingsFrozen)
      throw new FirestoreError(O.FAILED_PRECONDITION, "Firestore has already been started and its settings can no longer be changed. You can only modify settings before calling any other methods on a Firestore object.");
    this._settings = new FirestoreSettingsImpl(e), this._emulatorOptions = e.emulatorOptions || {}, void 0 !== e.credentials && (this._authCredentials = function __PRIVATE_makeAuthCredentialsProvider(e2) {
      ...
      switch (e2.type) {
        ...
        case "provider":
          return e2.client;
        default:
          throw new FirestoreError(O.INVALID_ARGUMENT, "makeAuthCredentialsProvider failed due to invalid credential type");
      }
    }(e.credentials));
  }
  _getSettings() {
  ...
  _terminate() {
    return function __PRIVATE_removeComponents(e) {
      const t = nn.get(e);
      t && (__PRIVATE_logDebug("ComponentProvider", "Removing Datastore"), nn.delete(e), t.terminate());
...
function connectFirestoreEmulator(e, t, n, r = {}) {
  ...
  o.host !== rn && o.host !== a && __PRIVATE_logWarn("Host has been set in both settings() and connectFirestoreEmulator(), emulator host will be used.");
  ...
  if (!deepEqual(u, _) && (e._setSettings(u), s && (pingServer(`https://${a}`), updateEmulatorBanner("Firestore", true)), r.mockUserToken)) {
    ...
    if ("string" == typeof r.mockUserToken)
      ...
      if (!s2)
        throw new FirestoreError(O.INVALID_ARGUMENT, "mockUserToken must contain 'sub' or 'user_id' field!");
      n2 = new User(s2);
    }
...
function collection(e, t, ...n) {
  ...
  }
  {
    if (!(e instanceof DocumentReference || e instanceof CollectionReference))
      throw new FirestoreError(O.INVALID_ARGUMENT, "Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
...
function collectionGroup(e, t) {
  if (e = __PRIVATE_cast(e, Firestore$1), __PRIVATE_validateNonEmptyArgument("collectionGroup", "collection id", t), t.indexOf("/") >= 0)
    throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid collection ID '${t}' passed to function collectionGroup(). Collection IDs must not contain '/'.`);
  return new Query(
    e,
    /* converter= */
    null,
...
var __PRIVATE_AsyncQueueImpl = class {
  constructor(e = Promise.resolve()) {
    ...
    this.ju = () => {
      ...
      e2 && __PRIVATE_logDebug(on, "Visibility state changed to " + e2.visibilityState), this.y_.A_();
    }, this.Hu = e;
    const t = getDocument();
  ...
  async Zu() {
    if (0 !== this.Qu.length) {
      try {
        ...
        if (!__PRIVATE_isIndexedDbTransactionError(e))
          throw e;
        __PRIVATE_logDebug(on, "Operation failed with retryable error: " + e);
      }
...
function initializeFirestore(e, t, n) {
  ...
  if (r.isInitialized(n)) {
    ...
    if (deepEqual(i, t))
      return e2;
    throw new FirestoreError(O.FAILED_PRECONDITION, "initializeFirestore() has already been called with different options. To avoid this error, call initializeFirestore() with the same options as when it was originally called, or call getFirestore() to return the already initialized instance.");
  }
  if (void 0 !== t.cacheSizeBytes && void 0 !== t.localCache)
    throw new FirestoreError(O.INVALID_ARGUMENT, "cache and cacheSizeBytes cannot be specified at the same time as cacheSizeBytes willbe deprecated. Instead, specify the cache size in the cache object");
  if (void 0 !== t.cacheSizeBytes && -1 !== t.cacheSizeBytes && t.cacheSizeBytes < Mt)
    throw new FirestoreError(O.INVALID_ARGUMENT, "cacheSizeBytes must be at least 1048576");
...
function getFirestore(e, n) {
  const r = "object" == typeof e ? e : getApp(), i = "string" == typeof e ? e : n || _t, s = _getProvider(r, "firestore").getImmediate({
    identifier: i
  });
  if (!s._initialized) {
...
function __PRIVATE_setPersistenceProviders(e, t, n) {
  if ((e = __PRIVATE_cast(e, Firestore))._firestoreClient || e._terminated)
    throw new FirestoreError(O.FAILED_PRECONDITION, "Firestore has already been started and persistence can no longer be enabled. You can only enable persistence before calling any other methods on a Firestore object.");
  if (e._componentsProvider || e._getSettings().localCache)
    throw new FirestoreError(O.FAILED_PRECONDITION, "SDK cache is already specified.");
  e._componentsProvider = {
    _online: t,
    _offline: n
...
function clearIndexedDbPersistence(e) {
  if (e._initialized && !e._terminated)
    throw new FirestoreError(O.FAILED_PRECONDITION, "Persistence can only be cleared before a Firestore instance is initialized or after it is terminated.");
  const t = new __PRIVATE_Deferred();
...
var Bytes = class _Bytes {
  ...
  static fromBase64String(e) {
    try {
      ...
      throw new FirestoreError(O.INVALID_ARGUMENT, "Failed to construct data from Base64 string: " + e2);
    }
  }
  /**
...
var FieldPath = class {
  ...
  constructor(...e) {
    for (let t = 0; t < e.length; ++t)
      if (0 === e[t].length)
        throw new FirestoreError(O.INVALID_ARGUMENT, "Invalid field name at argument $(i + 1). Field names must not be empty.");
...
var GeoPoint = class {
  ...
  constructor(e, t) {
    if (!isFinite(e) || e < -90 || e > 90)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Latitude must be a number between -90 and 90, but was: " + e);
    if (!isFinite(t) || t < -180 || t > 180)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Longitude must be a number between -180 and 180, but was: " + t);
...
var __PRIVATE_ParseContextImpl = class ___PRIVATE_ParseContextImpl {
  ...
  constructor(e, t, n, r, i, s) {
    ...
    void 0 === i && this._c(), this.fieldTransforms = i || [], this.fieldMask = s || [];
  }
  get path() {
  ...
  uc(e) {
    var t;
    const n = null === (t = this.path) || void 0 === t ? void 0 : t.child(e), r = this.ac({
      path: n,
      cc: false
    });
    return r.lc(e), r;
  ...
  Tc(e) {
    return __PRIVATE_createError(e, this.settings.methodName, this.settings.Ic || false, this.path, this.settings.Ec);
  }
  ...
  lc(e) {
    ...
    if (__PRIVATE_isWrite(this.oc) && an.test(e))
...
function __PRIVATE_parseSetData(e, t, n, r, i, s = {}) {
  ...
  if (s.merge)
    ...
    for (const r2 of s.mergeFields) {
      ...
      if (!o.contains(i2))
        throw new FirestoreError(O.INVALID_ARGUMENT, `Field '${i2}' is specified in your field mask but missing from your input data.`);
...
var __PRIVATE_NumericIncrementFieldValueImpl = class ___PRIVATE_NumericIncrementFieldValueImpl extends FieldValue {
  ...
  _toFieldTransform(e) {
    const t = new __PRIVATE_NumericIncrementTransformOperation(e.serializer, toNumber(e.serializer, this.Rc));
    return new FieldTransform(e.path, t);
  }
  isEqual(e) {
    return e instanceof ___PRIVATE_NumericIncrementFieldValueImpl && this.Rc === e.Rc;
  }
};
...
function __PRIVATE_parseUpdateVarargs(e, t, n, r, i, s) {
  ...
  if (s.length % 2 != 0)
    throw new FirestoreError(O.INVALID_ARGUMENT, `Function ${t}() needs to be called with an even number of arguments that alternate between field names and values.`);
...
function __PRIVATE_parseData(e, t) {
  ...
  if (e instanceof FieldValue)
    return function __PRIVATE_parseSentinelFieldValue(e2, t2) {
      if (!__PRIVATE_isWrite(t2.oc))
        throw t2.Tc(`${e2._methodName}() can only be used with update() and set()`);
      if (!t2.path)
        throw t2.Tc(`${e2._methodName}() is not currently supported inside arrays`);
  ...
  if (
    ...
    return function __PRIVATE_parseArray(e2, t2) {
      ...
      let r = 0;
      for (const i of e2) {
        let e3 = __PRIVATE_parseData(i, t2.Pc(r));
  ...
  return function __PRIVATE_parseScalarValue(e2, t2) {
    ...
    if (e2 instanceof DocumentReference) {
      ...
      if (!r.isEqual(n))
        throw t2.Tc(`Document reference is for database ${r.projectId}/${r.database} but should be for database ${n.projectId}/${n.database}`);
      return {
            ...
            [ht]: {
              arrayValue: {
                values: e3.toArray().map((e4) => {
                  if ("number" != typeof e4)
                    throw t3.Tc("VectorValues must only contain numeric values.");
    ...
    throw t2.Tc(`Unsupported field value: ${__PRIVATE_valueDescription(e2)}`);
  }(e, t);
}
function __PRIVATE_parseObject(e, t) {
  ...
  return isEmpty(e) ? (
    ...
    const i = __PRIVATE_parseData(r, t.uc(e2));
    null != i && (n[e2] = i);
  }), {
    mapValue: {
      fields: n
...
function __PRIVATE_validatePlainObject(e, t, n) {
  if (!__PRIVATE_looksLikeJsonObject(n) || !function __PRIVATE_isPlainObject(e2) {
    ...
    throw "an object" === r ? t.Tc(e + " a custom object") : t.Tc(e + " " + r);
  }
}
...
function __PRIVATE_createError(e, t, n, r, i) {
  ...
  let a = "";
  return (s || o) && (a += " (found", s && (a += ` in field ${r}`), o && (a += ` in document ${i}`), a += ")"), new FirestoreError(O.INVALID_ARGUMENT, _ + e + a);
}
...
function __PRIVATE_validateHasExplicitOrderByForLimitToLast(e) {
  if ("L" === e.limitType && 0 === e.explicitOrderBy.length)
    throw new FirestoreError(O.UNIMPLEMENTED, "limitToLast() queries require specifying at least one orderBy() clause");
}
...
function query(e, t, ...n) {
  ...
  t instanceof AppliableConstraint && r.push(t), r = r.concat(n), function __PRIVATE_validateQueryConstraintArray(e2) {
    ...
    if (t2 > 1 || t2 > 0 && n2 > 0)
      throw new FirestoreError(O.INVALID_ARGUMENT, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...)` statement. For example: change `query(query, where(...), or(...))` to `query(query, and(where(...), or(...)))`.");
  }(r);
  for (const t2 of r)
    e = t2._apply(e);
  return e;
    ...
    const t = __PRIVATE_newUserDataReader(e.firestore), n = function __PRIVATE_newQueryFilter(e2, t2, n2, r, i, s, o) {
      ...
      if (i.isKeyField()) {
        if ("array-contains" === s || "array-contains-any" === s)
          throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid Query. You can't perform '${s}' queries on documentId().`);
        if ("in" === s || "not-in" === s) {
...
var QueryOrderByConstraint = class _QueryOrderByConstraint extends QueryConstraint {
  ...
  _apply(e) {
    const t = function __PRIVATE_newQueryOrderBy(e2, t2, n) {
      if (null !== e2.startAt)
        throw new FirestoreError(O.INVALID_ARGUMENT, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
      if (null !== e2.endAt)
        throw new FirestoreError(O.INVALID_ARGUMENT, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
...
function __PRIVATE_newQueryBoundFromDocOrFields(e, t, n, r) {
  if (n[0] = getModularInstance(n[0]), n[0] instanceof DocumentSnapshot$1)
    return function __PRIVATE_newQueryBoundFromDocument(e2, t2, n2, r2, i) {
      if (!r2)
        throw new FirestoreError(O.NOT_FOUND, `Can't use a DocumentSnapshot that doesn't exist for ${n2}().`);
      const s = [];
      for (const n3 of __PRIVATE_queryNormalizedOrderBy(e2))
        if (n3.field.isKeyField())
          ...
          if (__PRIVATE_isServerTimestamp(e3))
            throw new FirestoreError(O.INVALID_ARGUMENT, 'Invalid query. You are trying to start or end a query using a document for which the field "' + n3.field + '" is an uncommitted server timestamp. (Since the value of this field is unknown, you cannot start/end a query with it.)');
          if (null === e3) {
            const e4 = n3.field.canonicalString();
            throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid query. You are trying to start or end a query using a document for which the field '${e4}' (used as the orderBy) does not exist.`);
          }
          s.push(e3);
        }
    ...
    }(e._query, e.firestore._databaseId, t, n[0]._document, r);
  {
    ...
    return function __PRIVATE_newQueryBoundFromFields(e2, t2, n2, r2, i2, s) {
      ...
      if (i2.length > o.length)
        throw new FirestoreError(O.INVALID_ARGUMENT, `Too many arguments provided to ${r2}(). The number of arguments must be less than or equal to the number of orderBy() clauses`);
      const _ = [];
      for (let s2 = 0; s2 < i2.length; s2++) {
        const a = i2[s2];
        if (o[s2].field.isKeyField()) {
          if ("string" != typeof a)
            throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid query. Expected a string for document ID in ${r2}(), but got a ${typeof a}`);
          if (!__PRIVATE_isCollectionGroupQuery(e2) && -1 !== a.indexOf("/"))
            throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid query. When querying a collection and ordering by documentId(), the value passed to ${r2}() must be a plain document ID, but '${a}' contains a slash.`);
          const n3 = e2.path.child(ResourcePath.fromString(a));
          if (!DocumentKey.isDocumentKey(n3))
            throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid query. When querying a collection group and ordering by documentId(), the value passed to ${r2}() must result in a valid document path, but '${n3}' is not because it contains an odd number of segments.`);
...
function __PRIVATE_parseDocumentIdValue(e, t, n) {
  if ("string" == typeof (n = getModularInstance(n))) {
    if ("" === n)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Invalid query. When querying with documentId(), you must provide a valid document ID, but it was an empty string.");
    if (!__PRIVATE_isCollectionGroupQuery(t) && -1 !== n.indexOf("/"))
      throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid query. When querying a collection by documentId(), you must provide a plain document ID, but '${n}' contains a '/' character.`);
    const r = t.path.child(ResourcePath.fromString(n));
    if (!DocumentKey.isDocumentKey(r))
      throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid query. When querying a collection group by documentId(), the value provided must result in a valid document path, but '${r}' is not because it has an odd number of segments (${r.length}).`);
    return __PRIVATE_refValue(e, new DocumentKey(r));
  }
  if (n instanceof DocumentReference)
    return __PRIVATE_refValue(e, n._key);
  throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid query. When querying with documentId(), you must provide a valid string or a DocumentReference, but it was: ${__PRIVATE_valueDescription(n)}.`);
}
function __PRIVATE_validateDisjunctiveFilterElements(e, t) {
  if (!Array.isArray(e) || 0 === e.length)
    throw new FirestoreError(O.INVALID_ARGUMENT, `Invalid Query. A non-empty array is required for '${t.toString()}' filters.`);
}
function __PRIVATE_validateNewFieldFilter(e, t) {
  ...
  if (null !== n)
    throw n === t.op ? new FirestoreError(O.INVALID_ARGUMENT, `Invalid query. You cannot use more than one '${t.op.toString()}' filter.`) : new FirestoreError(O.INVALID_ARGUMENT, `Invalid query. You cannot use '${t.op.toString()}' filters with '${n.toString()}' filters.`);
}
function __PRIVATE_validateQueryFilterConstraint(e, t) {
  if (!(t instanceof QueryFieldFilterConstraint || t instanceof QueryCompositeFilterConstraint))
    throw new FirestoreError(O.INVALID_ARGUMENT, `Function ${e}() requires AppliableConstraints created with a call to 'where(...)', 'or(...)', or 'and(...)'.`);
}
var AbstractUserDataWriter = class {
  ...
  convertVectorValue(e) {
    var t, n, r;
    const i = null === (r = null === (n = null === (t = e.fields) || void 0 === t ? void 0 : t[ht].arrayValue) || void 0 === n ? void 0 : n.values) || void 0 === r ? void 0 : r.map((e2) => __PRIVATE_normalizeNumber(e2.doubleValue));
...
var QuerySnapshot = class {
  ...
  docChanges(e = {}) {
    ...
    if (t && this._snapshot.excludesMetadataChanges)
      throw new FirestoreError(O.INVALID_ARGUMENT, "To include metadata changes with your document changes, you must also pass { includeMetadataChanges:true } to onSnapshot().");
...
function onSnapshot(e, ...t) {
  ...
  return function __PRIVATE_firestoreClientListen(e2, t2, n2, r2) {
    ...
    return e2.asyncQueue.enqueueAndForget(async () => __PRIVATE_eventManagerListen(await __PRIVATE_getEventManager(e2), s2)), () => {
      i2.yu(), e2.asyncQueue.enqueueAndForget(async () => __PRIVATE_eventManagerUnlisten(await __PRIVATE_getEventManager(e2), s2));
    };
...
var WriteBatch = class {
  ...
  _verifyNotCommitted() {
    if (this._committed)
      throw new FirestoreError(O.FAILED_PRECONDITION, "A write batch can no longer be used after commit() has been called.");
  }
};
function __PRIVATE_validateReference(e, t) {
  if ((e = getModularInstance(e)).firestore !== t)
    throw new FirestoreError(O.INVALID_ARGUMENT, "Provided document reference is from a different Firestore instance.");
  return e;
}
...
function runTransaction(e, t, n) {
  ...
  const r = Object.assign(Object.assign({}, cn), n);
  !function __PRIVATE_validateTransactionOptions(e2) {
    if (e2.maxAttempts < 1)
      throw new FirestoreError(O.INVALID_ARGUMENT, "Max attempts must be at least 1");
  }(r);
  ...
  const r = function __PRIVATE_parseIndexes(e2) {
    const t2 = "string" == typeof e2 ? function __PRIVATE_tryParseJson(e3) {
      try {
        ...
        throw new FirestoreError(O.INVALID_ARGUMENT, "Failed to parse JSON: " + (null == e4 ? void 0 : e4.message));
      }
    }(e2) : e2, n2 = [];
...
function __PRIVATE_tryGetString(e, t) {
  if ("string" != typeof e[t])
    throw new FirestoreError(O.INVALID_ARGUMENT, "Missing string value for: " + t);
  return e[t];
}
...
var __PRIVATE_TestingHooksSpiImpl = class ___PRIVATE_TestingHooksSpiImpl {
  ...
  static get instance() {
    return hn || (hn = new ___PRIVATE_TestingHooksSpiImpl(), function __PRIVATE_setTestingHooksSpi(e) {
      if (ft)
  ...
  !function __PRIVATE_setSDKVersion(e2) {
    ...
    const i = e2.getProvider("app").getImmediate(), s = new Firestore(new __PRIVATE_FirebaseAuthCredentialsProvider(e2.getProvider("auth-internal")), new __PRIVATE_FirebaseAppCheckTokenProvider(i, e2.getProvider("app-check-internal")), function __PRIVATE_databaseIdFromApp(e3, t2) {
      if (!Object.prototype.hasOwnProperty.apply(e3.options, ["projectId"]))
        throw new FirestoreError(O.INVALID_ARGUMENT, '"projectId" not provided in firebase.initializeApp.');
      return new DatabaseId(e3.options.projectId, t2);
    }(i, n), i);
    return r = Object.assign({
      useFetchStreams: t
    }, r), s._setSettings(r), s;
  }, "PUBLIC").setMultipleInstances(true)), registerVersion(C, F, e), // BUILD_TARGET will be replaced by values like esm2017, cjs2017, etc during the compilation
